 ...
 *
 * Modified by Sagemcom under LGPL license on 07/04/2009 
 * Copyright (c) 2010 Sagemcom All rights reserved.
 * Integration of ASF depayloader.
 *
 *
...
#define SERVER_CISCO_PREFIX	"Cisco-CDS/"
    ...
    GstRTSPMessage * resp)
{
  ...
  switch (req->type_data.request.method) {
    ...
    case GST_RTSP_OPTIONS:
    {
      ...
      if (server && (g_str_has_prefix (server, SERVER_PREFIX) || g_str_has_prefix (server, SERVER_CISCO_PREFIX)))
	  ctx->active = TRUE;
      else
	  ctx->active = FALSE;
      break;
    ...
    GstStructure * props)
{
  ...
  GST_DEBUG_OBJECT (ctx, "Parse SDP answer %i", ctx->active);
