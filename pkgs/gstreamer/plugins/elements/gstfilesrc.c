 ...
 *
 * Modified by Sagemcom under LGPL license on 02/12/2010 
...
#include <dirent.h>
static int selectAbsFiles(const struct dirent * p_dirent)
{
	if (!p_dirent) return 0;
	char *ptr = NULL;
	/*
	 * do not select relative paths: local and parent directories
	 */
	if (
	(ptr=strrchr(p_dirent->d_name,'.'))!=NULL
	&& (!strcmp(ptr,".") || !strcmp(ptr,".."))
	)
	{
		return 0;
	}
    ...
    GstBuffer ** buffer)
{
  ...
  GST_LOG_OBJECT(src, "check that file source (fd=%d,filename=<%s>) is still available",src->fd,src->filename);
  int result=1;
  if (access(src->filename,F_OK|R_OK)==0) {
  	char *fname = strdup(src->filename);
	if (fname!=NULL){
		char *dname = dirname((char*)fname);
		if (access(dname,F_OK|R_OK)==0)
		{
			struct dirent **namelist;
			int n,i;
			n = scandir(dname, &namelist, selectAbsFiles, alphasort);
			if (n >= 0)
			{
				for (i=0;i<n;i++)
				{
					GST_LOG ("elm %d:<%s>",i,namelist [i]->d_name);
					free (namelist [i]);
				}
				free (namelist);
			}
  ...
  if (result==1)
  {
  	GST_LOG ("it seems that source is empty or disconnected, stop map.");
  	goto could_not_mmap;
  }
  result=0;
