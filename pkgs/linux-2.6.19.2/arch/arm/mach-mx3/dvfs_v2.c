 ...
 * will be sent if DVFS counter value will be more than this value. 30
 */
#define DVFS_UPTHR		(37 << MXC_CCM_LTR0_UPTHR_OFFSET)

/*
 * Frequency decrease threshold. Decrease frequency change request
 * will be sent if DVFS counter value will be less than this value. 18
 */
#define DVFS_DNTHR		(38 << MXC_CCM_LTR0_DNTHR_OFFSET)
...
static void set_gp_weight(int gp_id, u8 weight)
{
	...
	if (gp_id < 9) {
		...
		reg = (reg & ~(MXC_CCM_LTR2_WSW_MASK(gp_id - 9))) |
		    (weight << MXC_CCM_LTR2_WSW_OFFSET(gp_id - 9));
...
static irqreturn_t dvfs_irq(int irq, void *dev_id)
{
	...
	if (((dvsup == DVSUP_LOW) && (fsvai == FSVAI_FREQ_DECREASE)) ||
		...
		printk(KERN_WARNING "Something is wrong? dvsup = %d, fsvai = %d\n", dvsup, fsvai);
		return IRQ_HANDLED;
	}
