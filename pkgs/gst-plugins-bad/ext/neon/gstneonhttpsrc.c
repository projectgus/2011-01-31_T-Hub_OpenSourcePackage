 ...
 *
 * Modified by Sagemcom under LGPL license on 28/05/2008 
...
/* SAGEM */
static gboolean	gst_neonhttp_set_caps(GstBaseSrc *bsrc, GstCaps *new_caps);
...
gst_neonhttp_src_start (GstBaseSrc * bsrc)
{
  ...
  const gchar *content_type; /* SAGEM */
  gint res;
  ...
  if (src->iradio_mode) {
	...
	/* SAGEM: Set caps */
	gst_neonhttp_set_caps(bsrc, gst_caps_new_simple("application/x-icy",
  ...
  /* SAGEM: Set caps */
  if (src->icy_caps == NULL) {
    content_type = ne_get_response_header(src->request, "Content-Type");
    if (!content_type)
      goto no_content_type;

    if (!strcmp(content_type, "video/mpeg2"))
	content_type = "video/mpeg";
    else if (!strcmp(content_type, "audio/mp4"))
	content_type = "audio/x-m4a";
    else if (!strcmp(content_type, "video/avi") || !strcmp(content_type, "video/x-ms-avi"))
	content_type = "video/x-msvideo";
    else if (!strcmp(content_type, "audio/x-ms-wma"))
	content_type = "audio/x-ms-asf";
    else if (!strcmp(content_type, "video/x-ms-wmx") || !strcmp(content_type, "video/x-ms-wmv") || !strcmp(content_type, "video/x-ms-wms"))
	content_type = "video/x-ms-asf";
    else if (!strncmp(content_type, "application/force-download", 26))
    {
	const char	*content_disposition;
	char		*filename;
	char		*ext = NULL;

	filename = strstr(content_type, "name=");
	if (!filename)
	{
	    content_disposition = ne_get_response_header(src->request, "Content-Disposition");
	    if (!content_disposition)
		goto no_content_disposition;
	    if (!strncasecmp(content_disposition, "attachment;", 11))
	    {
		if ((filename = strstr(content_disposition, "filename=")) != NULL)
		    ext = strrchr(filename + 9, '.');
	    }
	}
	else
	{
	    ext = strrchr(filename + 5, '.');
	}
	...
	if (ext != NULL)
	{
		...
		content_type = "video/mpeg";
	    else if (!strncmp(ext, ".wmv", 4))
		content_type = "video/x-ms-asf";
	    else if (!strncmp(ext, ".wma", 4))
		content_type = "audio/x-ms-asf";
	}
    }
    gst_neonhttp_set_caps(bsrc, gst_caps_new_simple(content_type, NULL));
  }

  return TRUE;

  /* ERRORS */
no_content_type: /* SAGEM: No content-type specified in HTTP header */
  {
    GST_ELEMENT_ERROR (src, LIBRARY, INIT,
        (NULL), ("No content-type specified in HTTP header"));
    return FALSE;
  }
no_content_disposition: /* SAGEM: No content-disposition specified in HTTP header */
  {
    GST_ELEMENT_ERROR (src, LIBRARY, INIT,
        (NULL), ("No content-disposition specified in HTTP header"));
    return FALSE;
  }
init_failed:
  {
...
static gboolean		gst_neonhttp_set_caps(GstBaseSrc *bsrc, GstCaps *new_caps)
{
    ...
    if (src->icy_caps == NULL || !gst_caps_is_equal(new_caps, src->icy_caps)) 
    {
	gst_caps_replace(&(src->icy_caps), new_caps);
	GST_DEBUG_OBJECT(src, "Changing src pad caps to %" GST_PTR_FORMAT, src->icy_caps);
	gst_pad_set_caps(bsrc->srcpad, src->icy_caps);
    } 
    else 
    {
	/* Caps never changed */
