 ...
 * 10/12/2008:
 * - Deliver fix audio headset.
 * - Deliver anti-pop SW under flag. It does not work yet.
 * 02/09/2008:
 * - Add missing events in pmic
 * - Merge pmic audio with IMX27 pmic audio.
 * 08/10/2007:
 * - PMIC/Atlas communication bugfix
 */
 ...
 * Defines
 */
#define EVENT_MASK_0			0xe9ffdf /*!< or-ed all events */
...
#ifdef DO_ULOG
	ulog(ULOG_ATLAS_PMIC_READ, reg_num);
	ulog(ULOG_ATLAS_PMIC_READ_VALUE, *reg_val);
#endif
	return ret;
}
...
#ifdef DO_ULOG
	ulog(ULOG_ATLAS_PMIC_WRITE, reg_num);
	ulog(ULOG_ATLAS_PMIC_WRITE_VALUE, reg_val);
#endif
...
int pmic_init_registers(void)
{
	...
	printk( "PMIC: pmic_init_registers #1\n");
	CHECK_ERROR(pmic_write(REG_INTERRUPT_MASK_1, MXC_PMIC_FRAME_MASK));
	printk( "PMIC: pmic_init_registers #2\n");
	CHECK_ERROR(pmic_write(REG_INTERRUPT_STATUS_0, MXC_PMIC_FRAME_MASK));
	printk( "PMIC: pmic_init_registers #3\n");
	CHECK_ERROR(pmic_write(REG_INTERRUPT_STATUS_1, MXC_PMIC_FRAME_MASK));
#if defined(CONFIG_MACH_MX31HSV1)
	/* ADCSEL0 = 1, ADCSEL1 = 0: */ // SAGEM Added by LEROYO
	CHECK_ERROR(pmic_write_reg(REG_ARBITRATION_PERIPHERAL_AUDIO, 0x001000, 0x003000));
	printk( "PMIC 1 primary SPI interface, secondary SPI has no ADC access\n");
#endif //defined(CONFIG_MACH_MX31HSV1)
	printk( "PMIC: pmic_init_registers OK\n");	
	return PMIC_SUCCESS;
}
...
int pmic_event_unmask(type_event event)
{
...
#ifdef DO_ULOG
	ulog(ULOG_ATLAS_UNMASK_EVENT, event);
#endif
	...
	if ((event_bit & event_mask) == 0) {
		printk("Error: unmasking a reserved/unused event\n");
		return PMIC_ERROR;
	}
