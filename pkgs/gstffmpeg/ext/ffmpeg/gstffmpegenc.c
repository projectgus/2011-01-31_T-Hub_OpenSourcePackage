 ...
 *
 * Modified by Sagemcom under LGPL license on 24/04/2009 
 * Copyright (c) 2010 Sagemcom All rights reserved.
 * Add compatibility with ffmpeg-0.4.8 which is more stable than ffmpeg-0.5
 *
...
gst_ffmpegenc_register (GstPlugin * plugin)
{
  ...
  while (in_plugin) {
...
#else /* !K_FFMPEG_0_5 */
    GST_DEBUG ("Trying plugin %s", in_plugin->name);
#endif /* K_FFMPEG_0_5 */
    ...
    if (in_plugin->type == CODEC_TYPE_VIDEO) {
      ...
      sinkcaps = 
#if defined(K_FFMPEG_0_5)
	  gst_ffmpeg_codectype_to_audio_caps (NULL,
					      in_plugin->id, TRUE, in_plugin);
#else /* !K_FFMPEG_0_5 */
      gst_ffmpeg_codectype_to_caps (in_plugin->type, NULL, in_plugin->id, FALSE);
#endif /* K_FFMPEG_0_5 */
    }
    if (!sinkcaps) {
