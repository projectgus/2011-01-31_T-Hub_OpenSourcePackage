			    ...
			    struct snd_pcm_sync_ptr __user *_sync_ptr)
{
...
#ifdef UNDERRUN_WARNING_ON
	{
		...
		if (n<0)
		{
			printk("%s - Underrun detected n=%d hw_ptr=0x%x appl_ptr=0x%x\n",__FUNCTION__,n,(unsigned int)status->hw_ptr,(unsigned int)sync_ptr.c.control.appl_ptr);
		}
	}
#endif
