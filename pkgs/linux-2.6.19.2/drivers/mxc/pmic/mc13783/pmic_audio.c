 ...
 *
 * Modified by Sagemcom under GPL license on 08/10/2007
...
#include <asm/arch/pmic_external.h> /* For sensors */
...
EXPORT_SYMBOL(pmic_audio_get_headset_state);
...
PMIC_STATUS pmic_audio_set_autodetect(int val)
{
	...
	reg_mask = SET_BITS(regAUDIO_RX_0, HSDETEN, 1) | 
			   ...
			   SET_BITS(regAUDIO_RX_0, AHSREN, 1) |
			   SET_BITS(regAUDIO_RX_0, AHSLEN, 1);
	...
	{
		reg_write = SET_BITS(regAUDIO_RX_0, HSDETEN, 0) | 
	                SET_BITS(regAUDIO_RX_0, HSDETAUTOB, 1) |
					SET_BITS(regAUDIO_RX_0, BIASEN, 0) |
					SET_BITS(regAUDIO_RX_0, BIASSPEED, 1) |
					SET_BITS(regAUDIO_RX_0, HSLDETEN, 0)|
					SET_BITS(regAUDIO_RX_0, AHSREN, 1) |
			   		SET_BITS(regAUDIO_RX_0, AHSLEN, 1);
					
	}
	status = pmic_write_reg(REG_AUDIO_RX_0, reg_write, reg_mask);
	pr_debug("pmic_audio_set_autodetect(%d), status = %d\n",val,status);
#ifdef DUMP_REG
			    ...
			    const PMIC_AUDIO_SOURCE device)
{
	...
	if ((device == STEREO_DAC) && (stDAC.handleState == HANDLE_FREE)) {
		...
		pr_debug("%s - Open stereo DAC 0x%x\n",__FUNCTION__,(unsigned int)stDAC.handle);
		...
		*handle = stDAC.handle;
		...
		pr_debug("%s - Open voice codec 0x%x\n",__FUNCTION__,(unsigned int)vCodec.handle);
		...
		*handle = vCodec.handle;
		...
		pr_debug("%s - Open extStereoIn 0x%x\n",__FUNCTION__,(unsigned int)extStereoIn.handle);
...
PMIC_STATUS pmic_audio_close(const PMIC_AUDIO_HANDLE handle)
{
	...
	pr_debug("%s - Close audio channel 0x%x\n",__FUNCTION__,(unsigned int)handle);
	/* Exit the critical section. */
	up(&mutex);
				    ...
				    const PMIC_AUDIO_NUMSLOTS numSlots)
{
	...
	unsigned int SSI_NW_MASK = SET_BITS(regSSI_NETWORK, STDCSLOTS, 3);
	...
	if (handle == (PMIC_AUDIO_HANDLE) NULL) {
		...
		if ((handle == vCodec.handle) &&
			...
			if ((stDAC.handleState == HANDLE_IN_USE) &&
				...
				pr_debug("%s - The requested bus already in USE\n",__FUNCTION__);
				rc = PMIC_PARAMETER_ERROR;
			} else if ((masterSlave == BUS_MASTER_MODE)
				   && (numSlots != USE_4_TIMESLOTS)) {
				pr_debug
				    ("%s - mc13783 supports only 4 slots in Master mode\n",__FUNCTION__);
				rc = PMIC_NOT_SUPPORTED;
			} else if ((masterSlave == BUS_SLAVE_MODE)
				   && (numSlots != USE_4_TIMESLOTS)) {
				pr_debug
				    ("%s - Driver currently supports only 4 slots in Slave mode\n",__FUNCTION__);
				rc = PMIC_NOT_SUPPORTED;
			} else if (!((protocol == NETWORK_MODE) ||
				     (protocol == I2S_MODE))) {
				pr_debug
				    ("%s - mc13783 Voice codec works only in Network and I2S modes\n",__FUNCTION__);
				rc = PMIC_NOT_SUPPORTED;
			} else {
				pr_debug
				    ("%s - Proceeding to configure Voice Codec\n",__FUNCTION__);
				...
				pr_debug
				    ("%s - mc13783 Voice codec successfully configured\n",__FUNCTION__);
				rc = PMIC_SUCCESS;
				//}
			...
			if ((vCodec.handleState == HANDLE_IN_USE) &&
				...
				rc = PMIC_PARAMETER_ERROR;
				   ...
				   && ((numSlots != USE_2_TIMESLOTS) &&
					(numSlots != USE_1_TIMESLOT))) {
				pr_debug
				    ("%s - STDAC uses only 1 or 2 slots in Normal and I2S modes\n",__FUNCTION__);
				rc = PMIC_PARAMETER_ERROR;
			} else if ((protocol == NETWORK_MODE) &&
				   !((numSlots == USE_1_TIMESLOT) ||
				     (numSlots == USE_2_TIMESLOTS) ||
				...
				pr_debug
				    ("%s - STDAC uses only 2,4 or 8 slots in Network mode\n",__FUNCTION__);
				rc = PMIC_PARAMETER_ERROR;
			} else if (protocol == SPD_IF_MODE) {
				pr_debug
				    ("%s - STDAC driver currently does not support SPD IF mode\n",__FUNCTION__);
				rc = PMIC_NOT_SUPPORTED;
			} else {
				pr_debug
				    ("%s - Proceeding to configure Stereo DAC\n",__FUNCTION__);
				if (busID == AUDIO_DATA_BUS_1) {
					reg_value =
				...
				if (pmic_write_reg
					...
					if (pmic_write_reg
						...
						pr_debug
						    ("%s - mc13783 Stereo DAC successfully configured\n",__FUNCTION__);
						rc = PMIC_SUCCESS;
					}
				}
			...
			pr_debug("%s - Handles only STDAC and VCODEC\n",__FUNCTION__);
		}
...
// Plus besoin car fait a l'init ...
//  rc = pmic_audio_set_autodetect(1);
//	if (rc != PMIC_SUCCESS) {
//		pr_debug("%s - Failed to enable headphone autodetect\n",__FUNCTION__);
//		rc = PMIC_ERROR;
//		goto exit;
//	}
	...
	if ((handle == stDAC.handle) && (stDAC.handleState == HANDLE_IN_USE)) {
		...
		if (rc != PMIC_SUCCESS) {
			pr_debug("%s - Failed to enable the Stereo DAC\n",__FUNCTION__);
			rc = PMIC_ERROR;
		}
	} else if ((handle == vCodec.handle)
		   && (vCodec.handleState == HANDLE_IN_USE)) {
		...
		rc = pmic_write_reg(REG_AUDIO_CODEC, VCODEC_ENABLE,
				    VCODEC_ENABLE);
		if (rc != PMIC_SUCCESS) {
			pr_debug("%s - Failed to enable the Voice codec\n",__FUNCTION__);
			rc = PMIC_ERROR;
		}
	}
	/* Exit critical section. */
...
PMIC_STATUS pmic_audio_disable(const PMIC_AUDIO_HANDLE handle)
{
	...
	if (rc == PMIC_SUCCESS) {
		pr_debug("%s - Disabled successfully\n",__FUNCTION__);
	}
	/* Exit critical section. */
	up(&mutex);
	return rc;
 ...
 */
PMIC_STATUS pmic_audio_set_callback(PMIC_AUDIO_CALLBACK func,
				    const PMIC_AUDIO_EVENTS eventMask)
{
	 ...
	 */
	pr_debug("%s - Enter eventMask=0x%x\n",__FUNCTION__,eventMask);
	...
	if (eventMask & MICROPHONE_DETECTED) {
		...
		eventNotify.func = (void *)func;
		eventNotify.param = (void *)(CORE_EVENT_MC2BI);
		rc = pmic_event_subscribe(EVENT_MC2BI, eventNotify);

		if (rc != PMIC_SUCCESS) {
			printk
			    ("%s: pmic_event_subscribe() for event MICROPHONE_DETECTED"
			     "failed\n", __FILE__);
			goto End;
		}

		pr_debug("Registered for event MICROPHONE_DETECTED\n");
	}
	...
	if (eventMask & HEADSET_DETECTED) {
		...
		if (rc != PMIC_SUCCESS) {
			printk
			    ("%s: pmic_event_subscribe() for event HEADSET_DETECTED "
			     "failed\n", __FILE__);
			goto Cleanup_HDT;
		}

		pr_debug("Registered for event HEADSET_DETECTED\n");
	}
	if (eventMask & HEADSET_STEREO) {
		...
		if (rc != PMIC_SUCCESS) {
			printk
			    ("%s: pmic_event_subscribe() for event HEADSET_STEREO "
			     "failed\n", __FILE__);
			goto Cleanup_HST;
		}
		pr_debug("Registered for event HEADSET_STEREO\n");
	}
	if (eventMask & HEADSET_THERMAL_SHUTDOWN) {
		...
		if (rc != PMIC_SUCCESS) {
			printk
			    ("%s: pmic_event_subscribe() for event HEADSET_THERMAL_SHUTDOWN "
			     "failed\n", __FILE__);
			goto Cleanup_TSD;
		}
		pr_debug("Registered for event HEADSET_THERMAL_SHUTDOWN\n");
	}
	if (eventMask & HEADSET_SHORT_CIRCUIT) {
		...
		if (rc != PMIC_SUCCESS) {
			printk
			    ("%s: pmic_event_subscribe() for event HEADSET_SHORT_CIRCUIT "
			     "failed\n", __FILE__);
			goto Cleanup_HShort;
		}
		pr_debug("Registered for event HEADSET_SHORT_CIRCUIT\n");
	}
				      ...
				      rampSpeed)
{
	...
	pr_debug("%s - ramSpeed=%d\n",__FUNCTION__,rampSpeed);
	/*
...
PMIC_STATUS pmic_audio_digital_filter_reset(const PMIC_AUDIO_HANDLE handle)
{
	...
	if ((handle == stDAC.handle) && (stDAC.handleState == HANDLE_IN_USE)) {
		...
		if (pmic_write_reg(REG_AUDIO_STEREO_DAC, reg_mask,
			...
			printk("%s - ERROR - STDAC filter reset\n",__FUNCTION__);
			rc = PMIC_ERROR;
		} else {
			pr_debug("%s - STDAC filter reset\n",__FUNCTION__);
		}
		...
		if (pmic_write_reg(REG_AUDIO_CODEC, reg_mask,
				   reg_mask) != PMIC_SUCCESS) {
			printk("%s - ERROR - CODEC filter reset\n",__FUNCTION__);
			rc = PMIC_ERROR;
		} else {
			pr_debug("%s - CODEC filter reset\n",__FUNCTION__);
		}
	}
	return rc;
}
...
void pmic_audio_dump_registers(void)
{
	...
	if (pmic_read_reg(REG_AUDIO_CODEC, &reg_value, REG_FULLMASK)
		...
		printk("Failed to read audio codec\n");
	}
	...
	if (pmic_read_reg
		...
		printk("Failed to read Stereo DAC\n");
	}
	...
	if (pmic_read_reg
		...
		printk("Failed to read SSI network\n");
	}
	...
	if (pmic_read_reg(REG_AUDIO_RX_0, &reg_value, REG_FULLMASK)
		...
		printk("Failed to read audio RX 0\n");
	}
	...
	if (pmic_read_reg(REG_AUDIO_RX_1, &reg_value, REG_FULLMASK)
		...
		printk("Failed to read audio RX 1\n");
	}
	...
	if (pmic_read_reg(REG_AUDIO_TX, &reg_value, REG_FULLMASK) ==
		...
		printk("Failed to read audio TX\n");
	}
	...
	if (handle == (PMIC_AUDIO_HANDLE) NULL) {
		...
		if ((clockIn != CLOCK_IN_CLIA) && (clockIn != CLOCK_IN_CLIB)) {
			...
			if (pmic_write_reg
			    (REG_AUDIO_CODEC, reg_value,
			     reg_mask) != PMIC_SUCCESS) {
				printk("%s - ERROR - CODEC clock set\n",__FUNCTION__);
				rc = PMIC_ERROR;
			} else {
				pr_debug("%s - CODEC clock set\n",__FUNCTION__);
		   ...
		   rc = PMIC_PARAMETER_ERROR;
			...
			if (pmic_write_reg
			    (REG_AUDIO_STEREO_DAC, reg_value,
			     reg_mask) != PMIC_SUCCESS) {
				printk("%s - ERROR - STDAC clock set\n",__FUNCTION__);
				rc = PMIC_ERROR;
			} else {
				pr_debug("%s - STDAC clock set\n",__FUNCTION__);
				rc = PMIC_SUCCESS;
		...
		if ((timeslot == USE_TS0_TS1) || (timeslot == USE_TS2_TS3)
			...
			if (pmic_write_reg
			    (REG_AUDIO_SSI_NETWORK, timeslot,
			     reg_mask) != PMIC_SUCCESS) {
				printk("%s - ERROR - STDAC primary timeslot set\n",__FUNCTION__);
				rc = PMIC_ERROR;
			} else {
				pr_debug("%s - STDAC primary timeslot set\n",__FUNCTION__);
	...
	if ((handle == vCodec.handle) && (vCodec.handleState == HANDLE_IN_USE)) {
		if (!((leftChannel == NO_MIC) || (leftChannel == MIC1_LEFT))) {
			...
			if (rightChannel == NO_MIC) {
				pr_debug("%s - rightChannel == NO_MIC\n",__FUNCTION__);
				...
				pr_debug("%s - rightChannel == MIC1_RIGHT_MIC_MONO\n",__FUNCTION__);
				reg_mask |= SET_BITS(regAUDIO_TX, AMC1REN, 1) |
				    SET_BITS(regAUDIO_TX, RXINREC, 1) |
				    SET_BITS(regAUDIO_TX, AMC2EN, 1) |
#if defined(CONFIG_MACH_MX27MEDIAPHONE) || defined(CONFIG_MACH_MX31HSV1)
				    SET_BITS(regAUDIO_TX, AMC1REN, 1) |
				    SET_BITS(regAUDIO_TX, ATXINEN, 1) |
				    SET_BITS(regAUDIO_TX, AMC1RITOV, 1) |
				    SET_BITS(regAUDIO_TX, AMC2ITOV, 1);
#else
				    SET_BITS(regAUDIO_TX, ATXINEN, 1);
#endif // #if defined(CONFIG_MACH_MX27MEDIAPHONE) || defined(CONFIG_MACH_MX31HSV1)
				...
				pr_debug("%s - rightChannel == MIC2_AUX\n",__FUNCTION__);
				reg_mask |= SET_BITS(regAUDIO_TX, AMC1REN, 1) |
				    SET_BITS(regAUDIO_TX, RXINREC, 1) |
				    SET_BITS(regAUDIO_TX, AMC2EN, 1) |
				    SET_BITS(regAUDIO_TX, ATXINEN, 1);
				...
				pr_debug("%s - rightChannel == LINE_IN\n",__FUNCTION__);
				reg_mask |= SET_BITS(regAUDIO_TX, AMC1REN, 1) |
				    SET_BITS(regAUDIO_TX, RXINREC, 1) |
				    SET_BITS(regAUDIO_TX, AMC2EN, 1) |
				    SET_BITS(regAUDIO_TX, ATXINEN, 1);
			...
			if (reg_mask == 0) {
				...
				if (rc == PMIC_SUCCESS) {
					pr_debug
					    ("%s - MIC inputs configured successfully\n",__FUNCTION__);
					     ...
					     rightChannel)
{
	...
	pr_debug("%s - Enter - leftChannel=%d rightChannel=%d\n",__FUNCTION__,leftChannel,rightChannel);
	/* Use a critical section to ensure a consistent hardware state. */
	if (down_interruptible(&mutex))
		return PMIC_SYSTEM_ERROR_EINTR;
	...
	if ((handle == vCodec.handle) && (vCodec.handleState == HANDLE_IN_USE)) {
		...
		if ((curr_left == NO_MIC) && (curr_right == NO_MIC)) {
			...
			if (reg_mask == 0) {
				...
				if (rc == PMIC_SUCCESS) {
					pr_debug
					    ("%s - MIC states configured successfully\n",__FUNCTION__);
					vCodec.leftChannelMic.micOnOff =
					    leftChannel;
					      ...
					      rightChannelGain)
{
	...
	pr_debug("%s - leftChannelMode=%d leftChannelGain=%d dB rightChannelMode=%d rightChannelGain=%d dB\n",__FUNCTION__,leftChannelMode,((int)leftChannelGain)-8,rightChannelMode,((int)rightChannelGain)-8);
	if ((handle == vCodec.handle) && (vCodec.handleState == HANDLE_IN_USE)) {
		if (!(((leftChannelGain >= MIC_GAIN_MINUS_8DB)
			...
			rc = PMIC_PARAMETER_ERROR;
			pr_debug("%s - VCODEC set record gain - wrong gain value\n",__FUNCTION__);
			...
			rc = PMIC_PARAMETER_ERROR;
			pr_debug("%s -VCODEC set record gain - wrong amp mode\n",__FUNCTION__);
		} else {
			if (vCodec.leftChannelMic.mic == MIC1_LEFT) {
				pr_debug("%s - vCodec.leftChannelMic.mic == MIC1_LEFT\n",__FUNCTION__);
			...
			if (vCodec.rightChannelMic.mic == MIC1_RIGHT_MIC_MONO) {
				pr_debug("%s - vCodec.rightChannelMic.mic == MIC1_RIGHT_MIC_MONO\n",__FUNCTION__);
				reg_mask |=
				...
				pr_debug("%s - vCodec.rightChannelMic.mic == MIC2_AUX\n",__FUNCTION__);	
				...
				pr_debug("%s - vCodec.rightChannelMic.mic == TXIN_EXT\n",__FUNCTION__);
				reg_mask |= SET_BITS(regAUDIO_TX, PGATXR, 31);
					     ...
					     biasCircuit)
{
	...
	pr_debug("%s - Enter - biasCircuit=%d\n",__FUNCTION__,biasCircuit);
				       ...
				       const PMIC_AUDIO_OUTPUT_PORT port)
{
	...
	unsigned int reg_write = 0;
	unsigned int reg_mask = 0;
	int headset_state=0, headset_mono=0;
		
	rc = pmic_audio_get_headset_state(&headset_state,&headset_mono);
	if (rc != PMIC_SUCCESS)
	{
		return rc;
	}
	...
	if ((port == MONO_ALERT) || (port == MONO_EXTOUT)) {
		...
		if (((handle == stDAC.handle)
			...
			if (port & MONO_LOUDSPEAKER) {
				...
				reg_write |= SET_BITS(regAUDIO_RX_0, ALSPEN,1) | 
				             SET_BITS(regAUDIO_RX_0, ALSPREF,1) |
				             SET_BITS(regAUDIO_RX_0, ALSPSEL, 1);
				pr_debug("mono_loudspeaker, ");
			}
			...
			if (port & STEREO_HEADSET_RIGHT) {
				...
				reg_write |= SET_BITS(regAUDIO_RX_0, AHSREN, 1) | 
				             SET_BITS(regAUDIO_RX_0, AHSSEL, 1);
				pr_debug("stereo_headset_right, ");
			}
			if (port & STEREO_OUT_LEFT) {
				reg_mask |=  SET_BITS(regAUDIO_RX_0, ARXOUTLEN,1) | 
				             SET_BITS(regAUDIO_RX_0, ARXOUTSEL, 1);
				if (headset_state) 
				{
					reg_write |= SET_BITS(regAUDIO_RX_0, ARXOUTLEN, 0) |
				             	SET_BITS(regAUDIO_RX_0, ARXOUTSEL, 0);
				} else {
					reg_write |= SET_BITS(regAUDIO_RX_0, ARXOUTLEN, 1) |
				             	SET_BITS(regAUDIO_RX_0, ARXOUTSEL, 1);
				}
				pr_debug("stereo_out_left, ");
			}
			if (port & STEREO_OUT_RIGHT) {
				reg_mask |=  SET_BITS(regAUDIO_RX_0, ARXOUTREN,1) | 
				             SET_BITS(regAUDIO_RX_0, ARXOUTSEL, 1);
				
				if (headset_state) 
				{
					reg_write |= SET_BITS(regAUDIO_RX_0, ARXOUTREN, 0) |
				             SET_BITS(regAUDIO_RX_0, ARXOUTSEL, 0);
				} else
				{
					reg_write |= SET_BITS(regAUDIO_RX_0, ARXOUTREN, 1) |
				             SET_BITS(regAUDIO_RX_0, ARXOUTSEL, 1);
				}
				pr_debug("stereo_out_right, ");
			}
			if (port & STEREO_LEFT_LOW_POWER) {
				...
				pr_debug("stereo_left_low_power, ");
			}
		} else if ((handle == vCodec.handle)
			   && (vCodec.handleState == HANDLE_IN_USE)
			   && (audioOutput.vCodecOut = VCODEC_DIRECT_OUT)) {
			...
			if (port & STEREO_HEADSET_LEFT) {
				reg_mask |=  SET_BITS(regAUDIO_RX_0, AHSLEN, 1) |
				             SET_BITS(regAUDIO_RX_0, AHSSEL, 1);
				reg_write |= SET_BITS(regAUDIO_RX_0, AHSLEN, 1) | 
				             SET_BITS(regAUDIO_RX_0, AHSSEL, 0);
				pr_debug("(v)stereo_headset_left, ");
			}
			if (port & STEREO_HEADSET_RIGHT) {
				reg_mask |=  SET_BITS(regAUDIO_RX_0, AHSREN, 1) |
				             SET_BITS(regAUDIO_RX_0, AHSSEL, 1);
				reg_write |= SET_BITS(regAUDIO_RX_0, AHSREN, 1) |
				             SET_BITS(regAUDIO_RX_0, AHSSEL, 0);
				pr_debug("(v)stereo_headset_right, ");
			}
			if (port & STEREO_OUT_LEFT) {
				reg_mask |=  SET_BITS(regAUDIO_RX_0, ARXOUTLEN, 1) | 
				             SET_BITS(regAUDIO_RX_0, ARXOUTSEL, 1);
			...
			if (port & STEREO_OUT_RIGHT) {
				...
				pr_debug("(v)stereo_out_right, ");
			}
			if (port & MONO_CDCOUT) {
				reg_mask  |= SET_BITS(regAUDIO_RX_0, CDCOUTEN, 1);
				reg_write |= SET_BITS(regAUDIO_RX_0, CDCOUTEN, 1);
				pr_debug("(v)mono_cdout, ");
			}
		}

		if (reg_mask == 0) {
			pr_debug("...Failed (no ports requested)\n");
		} else {
			rc = pmic_write_reg(REG_AUDIO_RX_0, reg_write, reg_mask);
			...
			if (rc == PMIC_SUCCESS) {
				...
				pr_debug("...Failed (pmic_write error)\n");
			}
		}
	}
	/* Exit the critical section. */
	...
	if ((port == MONO_ALERT) || (port == MONO_EXTOUT)) {
		...
		if (handle == stDAC.handle
			...
			if (port & MONO_LOUDSPEAKER) {
				reg_mask |= SET_BITS(regAUDIO_RX_0,  ALSPEN,  1) |
				            SET_BITS(regAUDIO_RX_0,  ALSPREF, 1);
				reg_write |= SET_BITS(regAUDIO_RX_0, ALSPEN,  0) |
				             SET_BITS(regAUDIO_RX_0, ALSPREF, 0);
			...
			if (port & MONO_CDCOUT) {
				reg_mask  |= SET_BITS(regAUDIO_RX_0, CDCOUTEN, 1);
				reg_write |= SET_BITS(regAUDIO_RX_0, CDCOUTEN, 0);
			}
		}
					  ...
					  const PMIC_AUDIO_OUTPUT_PGA_GAIN gain)
{
	...
	pr_debug("%s - gain=%d dB (0x%x)\n",__FUNCTION__,(((int)gain)-11)*3,gain);
	...
	if (!((gain >= OUTPGA_GAIN_MINUS_33DB)
		...
		//SAGEM, gain 0, 1 and 2 all equal to -33dB, therefore we skip these values
		reg_gain = gain + 2;

		if ((handle == extStereoIn.handle) &&
		    (extStereoIn.handleState == HANDLE_IN_USE)) {
			pr_debug("set extStereoIn volume to %d dB\n",(((int)gain)-11)*3);
					  ...
					  rightGain)
{
	 ...
	 * global data.
	 */
	pr_debug("%s - leftGain= %d dB rightGain= %d dB\n",__FUNCTION__,((int)leftGain-7)*3,((int)rightGain-7)*3);
						...
						mode)
{
	...
	if ((mode >= MONO_ADDER_OFF) && (mode <= STEREO_OPPOSITE_PHASE)) {
		if (((handle == stDAC.handle) &&
			...
			if (rc == PMIC_SUCCESS) {
				pr_debug("%s - Output mono adder mode set\n",__FUNCTION__);
...
PMIC_STATUS pmic_audio_output_enable_phantom_ground(void)
{
	...
	const unsigned int reg_write = SET_BITS(regAUDIO_RX_0, HSPGDIS, 0);
...
PMIC_STATUS pmic_audio_output_disable_phantom_ground()
{
	PMIC_STATUS rc = PMIC_PARAMETER_ERROR;
	const unsigned int reg_mask  = SET_BITS(regAUDIO_RX_0, HSPGDIS, 1);
	const unsigned int reg_write = SET_BITS(regAUDIO_RX_0, HSPGDIS, 1);
	/* No critical section required here since we are not updating any
	 * global data.
	 */
	rc = pmic_write_reg(REG_AUDIO_RX_0, reg_write, reg_mask);
...
/*!
 * @brief Get the current state of the headset (plugged or unplugged)
 *
 * @param       is_headset           1 if the headset is plugged, 0 otherwise.
 * @param       is_mono              1 if the user plugged a monophonic headset,
 *                                   0 if this is a stereo headset
 *
 * @retval      PMIC_SUCCESS         If headset state could be retreived.
 * @retval      PMIC_ERROR           If headset state could not be retreived.
 */
PMIC_STATUS pmic_audio_get_headset_state(int *is_headset, int *is_mono)
{
	t_sensor_bits sensors = { false };
	pmic_get_sensors(&sensors);

	headsetState = ! sensors.sense_hsdets;
	if (is_headset != NULL)
	{	*is_headset = ! sensors.sense_hsdets;
	}	
	if (is_mono != NULL)
		*is_mono    = ! sensors.sense_hsls;

	return PMIC_SUCCESS;
}

/*!
 * @brief Get the last known state of the headset (plugged or unplugged)
 *
 * @retval      TRUE            If headset is plugged.
 * @retval      FALSE           If headset is absent.
 */
bool pmic_audio_get_last_headset_state(void)
{
	return headsetState == HEADSET_ON;
}

EXPORT_SYMBOL(pmic_audio_get_last_headset_state);
