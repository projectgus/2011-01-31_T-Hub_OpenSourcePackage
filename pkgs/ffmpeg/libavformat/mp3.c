 ...
 *
 * Modified by Sagemcom under LGPL license on 23/04/2009 
...
static void id3v2_parse(AVFormatContext *s, int len, uint8_t version, uint8_t flags)
{
    ...
    /*SAGEM : keep length in order to skip end of tag properly in case of errors */
    id3v2_len = len;
    ...
    while(len >= taghdrlen) {
        ...
        if(len < 0)
        {
            /* Skip to end of tag properly */
            av_log(s, AV_LOG_WARNING, "Force to skip to end of tag");
            url_fseek(&s->pb, (id3v2_len + taghdrlen), SEEK_SET);
            break;
        }
