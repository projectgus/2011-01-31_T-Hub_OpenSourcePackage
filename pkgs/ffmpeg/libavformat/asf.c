 ...
 *
 * Modified by Sagemcom under LGPL license on 10/07/2009 
...
static int asf_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    for(;;) {
        ...
        if (!memcmp(&g, &file_header, sizeof(GUID))) {
            ...
            if (asf->asfid2avid[st->id] == 0)
                id_stream1 = st->id;
            else if(asf->asfid2avid[st->id] == 1)
                id_stream2 = st->id;
...
//av_log(NULL, AV_LOG_ERROR, "%d %d %d %d %d <%s>\n", i, stream_num, name_len, value_type, value_len, name);
    ...
    for(i=1; i<128; i++){
        ...
        if( (stream_num>=0)&&(asf->stream_bitrates[i] > 0)
                                         &&(id_stream1!=-1)&&(id_stream2!=-1) ){
            AVStream * avSt;
            if( asf->stream_bitrates[id_stream1] == 0 ) {
#ifdef DEBUG
                printf("\nput stream id %d in first position (instead of stream id:%d)\n", i, id_stream1);
#endif
                asf->asfid2avid[id_stream1] = stream_num;
                asf->asfid2avid[i] = 0;
                avSt = s->streams[0];
                s->streams[0] = s->streams[stream_num];
                s->streams[stream_num] = avSt;
                if(stream_num == 1)
                    id_stream2 = id_stream1;
                id_stream1 = i;
            } else if (( asf->stream_bitrates[id_stream2] == 0 )&&(stream_num!=0)) {
#ifdef DEBUG
                printf("\nput stream id %d in second position (instead of stream id:%d)\n", i, id_stream2);
#endif
                asf->asfid2avid[id_stream2] = stream_num;
                asf->asfid2avid[i] = 1;
                avSt = s->streams[1];
                s->streams[1] = s->streams[stream_num];
                s->streams[stream_num] = avSt;
                id_stream2 = i;
            }
        }
