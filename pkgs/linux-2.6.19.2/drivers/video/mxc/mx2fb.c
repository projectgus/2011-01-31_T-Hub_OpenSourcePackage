...
extern void gpio_inverter_enable(void);
extern void gpio_inverter_disable(void);
extern void enable_lcd_test_mode(void);
extern void disable_lcd_test_mode(void);

#include "asm-arm/arch-mxc/pmic_light.h"
#define DEFAULT_CURRENT_1 1
#define DEFAULT_CURRENT_2 1
#define DEFAULT_PWM_1     7
#define DEFAULT_PWM_2     15
#define CHANNEL_PWM1      BACKLIGHT_LED2
#define BANK_PWM2         TCLED_FUN_BANK3
#define CHANNEL_PWM2      TCLED_IND_BLUE
#define CYCLE_TIME        0
...
#ifdef CONFIG_MACH_MX27MEDIAPHONE
static unsigned char brightness = 190;
#else
...
#ifdef CONFIG_MACH_MX27MEDIAPHONE
	case MX2FB_SET_TEST_MODE:
	  enable_lcd_test_mode();
	  break;
	case MX2FB_OUT_TEST_MODE:
	  disable_lcd_test_mode();
	  break;
#endif // CONFIG_MACH_MX27MEDIAPHONE
#ifdef CONFIG_FB_MXC_TVOUT
...
static void _enable_lcdc(struct fb_info *info)
{
...
#ifdef CONFIG_MACH_MX27MEDIAPHONE
	int start_count=0;
	unsigned short inv_cur[8];
	int inverter_threshold;

	// After experimental measurements, the link between brightness level and inverter threshold is this

	inverter_threshold = brightness * 4/3;
#endif
	...
	if (mx2fbi->type == MX2FB_TYPE_GW)
		...
		msleep(300);
#if 0
		pmic_adc_convert_8x(TS_Y_POS1,inv_cur);
		if (*inv_cur < inverter_threshold){
		  do {
		    start_count++;
		    printk("start count: %i\n",start_count);
		    
		    // We only want to try threee times with a brightness level of 70%  

		    if (start_count == 11) break;

		    // Following the hardware recommendations we disable inverter during 500 ms then reenable it and wait 300 ms before new measure

		    gpio_inverter_disable();
		    msleep(500);
		    gpio_inverter_enable();
		    msleep(300);
		    pmic_adc_convert_8x(TS_Y_POS1,inv_cur);
		    
		  }  while (*inv_cur < inverter_threshold);
		}

		// if the current is still zero, we try to set the luma level to 100 %
		...
		if (*inv_cur < inverter_threshold)
		  {
		    ...
		    // Bring back the luma level to the wished level
		    _set_brightness(brightness);
...
#endif
		if (*inv_cur < inverter_threshold) {
		  printk("Inverter not started, no current detected\n");
		}
		else {
		  printk("Inverter activated, Inverter current: %x\n",*inv_cur);
		}
      ...
      { 
         ...
         if (  M_SAGEMBOARD_COMPMASK_IS_GA2(zz_board_type) )
         {
            ...
            pmic_bklit_set_current(CHANNEL_PWM1,DEFAULT_CURRENT_1); 
            pmic_bklit_set_dutycycle(CHANNEL_PWM1, DEFAULT_PWM_1); 
            pmic_bklit_set_cycle_time(CYCLE_TIME); 

            pmic_tcled_enable(TCLED_IND_MODE,TCLED_FUN_BANK3); 
            pmic_tcled_ind_set_current(CHANNEL_PWM2, DEFAULT_CURRENT_2, BANK_PWM2); 
            pmic_tcled_fun_set_dutycycle(TCLED_FUN_BANK3, CHANNEL_PWM2, DEFAULT_PWM_2); 
            pmic_tcled_fun_set_cycletime(BANK_PWM2, CYCLE_TIME); 
            //
         }
      }
...
#if defined(CONFIG_MACH_MX27MEDIAPHONE) || defined(CONFIG_MACH_MX31LSV0) 
        pcr |= (var->sync & FB_FLMPOL_ACT_HIGH) ? FB_FLMPOL_ACT_HIGH : 0;
        pcr |= (var->sync & FB_LPPOL_ACT_HIGH) ? FB_LPPOL_ACT_HIGH : 0;
        pcr |= (var->sync & FB_CLKPOL_ACT_POS) ? FB_CLKPOL_ACT_POS : 0;
#endif
