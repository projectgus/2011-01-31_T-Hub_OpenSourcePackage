...
_clutter_events_init (ClutterBackend *backend)
{
  ...
  if (event_source->ts_device)
      ...
      if (ts_config (event_source->ts_device))
	{
      ...
      /* Set no blocking mode on touchscreen device */
      if (fcntl(ts_fd(event_source->ts_device), F_GETFL, &mode) >= 0)
	  fcntl(ts_fd(event_source->ts_device), F_SETFL, mode | O_NONBLOCK);

#if defined(K_SAGEM_HOMESCREEN)
      g_source_set_priority (source, CLUTTER_PRIORITY_EVENTS + 30);
                       ...
                       gint    *timeout)
{
  ...
  *timeout = -1;
  retval = clutter_events_pending () || ((ClutterEventSource *)source)->clicked;
...
clutter_event_check (GSource *source)
{
  ...
  retval = ((event_source->event_poll_fd.revents & G_IO_IN) ||
            clutter_events_pending () || event_source->clicked);
  ...
  if ((!clutter_events_pending()) &&
        (ts_read(event_source->ts_device, &tsevent, 1) == 1))
    {
      ...
      */
      ...
      if (tsevent.pressure && !event_source->clicked)
        {
	  event->button.type = event->type = CLUTTER_BUTTON_PRESS;
          ...
          event_source->clicked = TRUE;
        }
      else if (tsevent.pressure && event_source->clicked)
        {
