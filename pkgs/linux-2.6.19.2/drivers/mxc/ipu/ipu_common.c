...
/* static uint32_t g_channel_init_mask_backup = 0; */ /* SAGEM LE ROY O */
...
/*		if (g_csi_used) {
		...
		} */ /* SAGEM LE ROY O: Not needed: camera related test workaround */

		mdelay(2); /* 2 ms  after bklt PWM is switched off*/
		gpio_lcd_inactive();
		g_ipu_config = __raw_readl(IPU_CONF);
		__raw_writel(0, IPU_CONF); /* clear all units' clocks and enables */
	}
	return 0;
}
...
/*		if (g_csi_used) {
		...
		__raw_writel(g_ipu_config, IPU_CONF); /* restore all units' clocks and enables */
		gpio_lcd_active();
	}
	return 0;
}
