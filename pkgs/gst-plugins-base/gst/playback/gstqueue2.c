...
#define DEFAULT_FORCE_QUEUE_FILLED FALSE
...
struct _GstQueue
{
  ...
  /* force queue to be filled before starting dequeue */
  gboolean force_queue_filled;
...
gst_queue_class_init (GstQueueClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_FORCE_QUEUE_FILLED,
      g_param_spec_boolean ("force-queue-filled", "Force Queue Filled",
          "Force queue to be filled (based on high-percent threshold) before starting the first dequeue",
          DEFAULT_FORCE_QUEUE_FILLED, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
...
gst_queue_init (GstQueue * queue, GstQueueClass * g_class)
{
  ...
  queue->force_queue_filled = DEFAULT_FORCE_QUEUE_FILLED;
...
update_buffering (GstQueue * queue)
{
  ...
  {
    queue->force_queue_filled = FALSE;
    return;
  }
...
#define GET_PERCENT(format) ((queue->max_level.format) > 0 ? \
  ...
  if (queue->is_buffering) {
    ...
    if (percent >= queue->high_percent)
    {
      ...
      queue->force_queue_filled = FALSE;
      GST_DEBUG_OBJECT (queue, "Unlock the dequeue");
    }
  } else {
...
gst_queue_loop (GstPad * pad)
{
  ...
  if(!queue->force_queue_filled)
  {
    ...
    GST_CAT_DEBUG_OBJECT (queue_dataflow, queue, "waiting queue to be full");
    ...
    guint prop_id, const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_HIGH_PERCENT:
      ...
      break;
    case PROP_FORCE_QUEUE_FILLED:
      queue->force_queue_filled = g_value_get_boolean (value);
      break;
    ...
    guint prop_id, GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_HIGH_PERCENT:
      ...
      break;
    case PROP_FORCE_QUEUE_FILLED:
      g_value_set_boolean (value, queue->force_queue_filled);
      break;
    case PROP_TEMP_LOCATION:
