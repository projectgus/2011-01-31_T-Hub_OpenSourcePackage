 ...
 *
 *
 *  Modified by Sagemcom under LGPL license on 26/10/2010 
 *  Copyright (c) 2010 Sagemcom All rights reserved.
 *  This patch is a bit obscure but it fixes browser crash when mixing audio from flash player and native player.
 *
 *
...
static int snd_pcm_dmix_status(snd_pcm_t *pcm, snd_pcm_status_t * status)
{
	...
	if((status != NULL)&&(status->state <= SNDRV_PCM_STATE_LAST ))
	{
	...
	{
		printf("\n WARNING: alsa-lib: snd_pcm_dmix_status, bad status->state\n");
		memset(&status->trigger_tstamp, 0, sizeof(status->trigger_tstamp));
		memset(&status->tstamp, 0, sizeof(status->tstamp));
		memset(&status->appl_ptr, 0, sizeof(status->appl_ptr));
		memset(&status->hw_ptr, 0, sizeof(status->hw_ptr));
		memset(&status->delay, 0, sizeof(status->delay));
		memset(&status->avail, 0, sizeof(status->avail));
		memset(&status->avail_max, 0, sizeof(status->avail_max));
		memset(&status->overrange, 0, sizeof(status->overrange));
		memset(&status->suspended_state, 0, sizeof(status->suspended_state));
