 ...
 * @param direction	GPIO_DIR_INPUT=1 for input; GPIO_DIR_OUTPUT=0 for output
 */

void mxc_set_gpio_direction(iomux_pin_name_t pin, gpio_direction_t direction)
{
	...
	spin_lock(&port->lock);
	_set_gpio_direction(port, GPIO_TO_INDEX(gpio), direction);
...
static inline int _request_gpio(struct gpio_port *port, u32 index)
{
	...
	pr_debug("Reserve GPIO port %d (0-based), pin %d\n",port->num, index);
...
#warning CLD: attention: Spurious interrupt trace will be hidden for prod ! bad at debug time. 
#if 0
 ...
 * Enable a gpio pin's interrupt by clearing the bit in the imr.
 * @param irq		a gpio virtual irq number
 * note : Cannot call set_irq_handler() since irq->lock is held
 */
int gpio_set_irq_type(u32 irq, u32 type)
{
	u32 gpio = MXC_IRQ_TO_GPIO(irq);
	struct gpio_port *port = get_gpio_port(gpio);
...
EXPORT_SYMBOL(gpio_set_irq_type);
