...
typedef enum {
	...
	CORE_EVENT_DP,		/*!< Detected DP connector event. */
	CORE_EVENT_DM,		/*!< Detected DM connector event. */
} PMIC_CORE_EVENT;
				      ...
				      const PMIC_CONVITY_EVENTS eventMask)
{
	...
	if ((handle == usb.handle) && (usb.handle_state == HANDLE_IN_USE)) {
		...
		if ((func == NULL) || (eventMask == 0)
			...
			if (eventMask & USB_DP_HI) {
				/* We need to register for the 4.4V interrupt. */
				//EVENT_USBI or EVENT_USB_44VI
				eventNotify.func = pmic_convity_event_handler;
				eventNotify.param = (void *)(CORE_EVENT_DP);
				rc = pmic_event_subscribe(EVENT_UDPM,
							  eventNotify);

				if (rc != PMIC_SUCCESS) {
					printk("%s - Subscribe has failed\n",__FUNCTION__);
					return rc;
				}
			}

			if (eventMask & USB_DM_HI) {
				/* We need to register for the 4.4V interrupt. */
				//EVENT_USBI or EVENT_USB_44VI
				eventNotify.func = pmic_convity_event_handler;
				eventNotify.param = (void *)(CORE_EVENT_DM);
				rc = pmic_event_subscribe(EVENT_UDMM,
							  eventNotify);
...
static void pmic_convity_tasklet(unsigned long arg)
{
	...
	if (eventID & CORE_EVENT_DP) {
		spin_lock_irqsave(&lock, flags);
		eventID &= ~CORE_EVENT_DP;
		spin_unlock_irqrestore(&lock, flags);

		activeEvents = USB_DP_HI;
		
	}
	else if (eventID & CORE_EVENT_DM) {
		spin_lock_irqsave(&lock, flags);
		eventID &= ~CORE_EVENT_DM;
		spin_unlock_irqrestore(&lock, flags);

		activeEvents = USB_DM_HI;
	}
	else if (eventID & CORE_EVENT_4V4) {
		spin_lock_irqsave(&lock, flags);
