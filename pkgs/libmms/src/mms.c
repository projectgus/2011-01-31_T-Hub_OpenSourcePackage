...
/*   lprintf("%d\n", fallback_io_select(data, socket, MMS_IO_READ_READY, 1000)); */
  ...
  while (len < num)
  {
    fd_set set;
    int res ;
    int timeout_msec= 19000;
    struct timeval tv = { timeout_msec / 1000, (timeout_msec % 1000) * 1000};
    FD_ZERO(&set);
    FD_SET(socket, &set);
    res = select(socket +1, &set , NULL, NULL, &tv);
    lprintf("select result:%d\n", res);
    if(res == 0)
        return -1;
          ...
          while( (j+24) < length ) {
            ...
            switch(guid) {
              case GUID_ASF_EXTENDED_STREAM_PROPERTIES:
                {
                  int bitrate_leak = LE_32(this->asf_header + i + j + 16);
                  ...
                  //lprintf ("ext_j: %d\n", ext_j);
                  // Get optional Stream Property Object
                  if( (ext_j+24) < l ) {
                    ...
                    lprintf ("No optional Stream Property Object: field not long enough\n");
                  }
...
int static mms_choose_best_streams(mms_io_t *io, mms_t *this) {
  ...
  int     max_arate      = 0;
  int     max_vrate      = 0;
  int     maxLimit_arate = this->max_audio_rate;
  int     maxLimit_vrate = this->max_video_rate;
  int     min_vrate      = 0;
  int     min_arate      = 0;
  int     stream_id;
//  int     bandwitdh_left;
  ...
  /* command 0x33 */
  /* SAGEM: choose the best quality for the audio stream and video stream, which don't overflow the choosen max limits*/
  lprintf("num_stream_ids=%d\n", this->num_stream_ids);
  lprintf("choose streams with maximum rates constraints: audio:%d, video:%d\n", maxLimit_arate, maxLimit_vrate);
  for (i = 0; i < this->num_stream_ids; i++) {
    stream_id = this->stream_ids[i];
    switch (this->stream_types[stream_id]) {
      case ASF_STREAM_TYPE_AUDIO:
        if ((this->bitrates[stream_id] > max_arate)&&(this->bitrates[stream_id] < maxLimit_arate)) {
          audio_stream = stream_id;
          max_arate = this->bitrates[stream_id];
        }
        break;
      case ASF_STREAM_TYPE_VIDEO:
        if ((this->bitrates[stream_id] > max_vrate)&&(this->bitrates[stream_id] < maxLimit_vrate)) {
          video_stream = stream_id;
          max_vrate = this->bitrates[stream_id];
        }
        break;
      default:
        break;
  ...
  /* choose the lower bitrate for audio */
  if (!audio_stream && this->has_audio) {
    for (i = 0; i < this->num_stream_ids; i++) {
      stream_id = this->stream_ids[i];
      switch (this->stream_types[stream_id]) {
        case ASF_STREAM_TYPE_AUDIO:
          if ((this->bitrates[stream_id] < min_arate) ||
              (!min_arate)) {
            audio_stream = stream_id;
            min_arate = this->bitrates[stream_id];
          }
          break;
        default:
          break;
  ...
  /* choose the lower bitrate for video */
  ...
  memset (this->scmd_body, 0, 50);
  
  /*SAGEM : disabled first stream if needed */
  if (!((this->stream_ids[0] == audio_stream) ||
      (this->stream_ids[0] == video_stream))) {
    lprintf("disabling stream %d\n", this->stream_ids[0]);
    this->scmd_body [ 0 ] = 0x02;
    this->scmd_body [ 1 ] = 0x00;
    
    /* forces the asf demuxer to not choose this stream */
    if (this->bitrates_pos[this->stream_ids[0]]) {
      this->asf_header[this->bitrates_pos[this->stream_ids[0]]]     = 0;
      this->asf_header[this->bitrates_pos[this->stream_ids[0]] + 1] = 0;
      this->asf_header[this->bitrates_pos[this->stream_ids[0]] + 2] = 0;
      this->asf_header[this->bitrates_pos[this->stream_ids[0]] + 3] = 0;
    }
  }
...
/* SAGEM: add max_audio_rate and max_video_rate arguments */
mms_t *mms_connect (mms_io_t *io, void *data, const char *url, int bandwidth, int max_audio_rate, int max_video_rate) {
#ifdef USE_ICONV
  iconv_t url_conv;
#else
  int     url_conv = 0;
  ...
  this->max_video_rate  = max_video_rate;
  this->current_pos     = 0;
  this->eos             = 0;
    ...
    case 0x06:
      {
        ...
        /* Get stream duration here */
        this->duration = LE_64(this->buf + CMD_HEADER_LEN + CMD_PREFIX_LEN + 24);
...
static int get_media_packet (mms_io_t *io, mms_t *this) {
  ...
  switch (get_packet_header (io, this, &header)) {
    case -1:
      /* connection timeout*/
      lprintf ("mms: get_packet_header: connection timeout\n");
      return -1;
      break;
    case MMS_PACKET_ERR:
  /* FIXME: de-xine-ification */
	...
	if(this->need_discont)
	{
	  ...
	  ((off_t)header.packet_seq - this->start_packet_seq) * (off_t)this->asf_packet_len;
        ...
        if (len == -1){
          lprintf ("mms: get_media_packet failed: connection timeout\n");
          return -1;
        }
        if (len != header.packet_len) {
	  /* FIXME: de-xine-ification */
...
/* return -1 if connection timeout with server occurs , length read otherwise*/
  ...
  while (total < len && !this->eos) {
    ...
    if (this->asf_header_read < this->asf_header_len) {
      ...
      if (bytes_left == 0) {
        ...
        if (packet_type == -1) {
          lprintf ("mms: Connection timeout with server\n");
          return -1;
        }
...
uint64_t mms_get_duration (mms_t *this) {
