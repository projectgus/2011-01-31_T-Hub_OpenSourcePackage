 ...
 *
 * Modified by Sagemcom under LGPL license on 20/03/2009 
...
#define DEFAULT_MAX_AUDIO_RATE    2147483
#define DEFAULT_MAX_VIDEO_RATE    2147483
...
gst_mms_class_init (GstMMSClass * klass)
{
  ...
  /* SAGEM */
  g_object_class_install_property (gobject_class, ARG_MAX_AUDIO_RATE,
      g_param_spec_uint ("max audio rate", "Max audio rate",
          "maximun acceptable audio rate,in kbps , to choose if case of multiple bitrate connection ",
          0, G_MAXINT / 1000, DEFAULT_MAX_AUDIO_RATE, G_PARAM_READWRITE));

  /*SAGEM */
  g_object_class_install_property (gobject_class, ARG_MAX_VIDEO_RATE,
      g_param_spec_uint ("max video rate", "Max video rate",
          "maximun acceptable video rate,in kbps , to choose if case of multiple bitrate connection ",
          0, G_MAXINT / 1000, DEFAULT_MAX_VIDEO_RATE, G_PARAM_READWRITE));
  ...
  switch (GST_QUERY_TYPE (query)) {
    ...
    case GST_QUERY_DURATION:
      ...
      if (format == GST_FORMAT_TIME) {
          if (mmssrc->connection)
              value = (gint64) mms_get_duration (mmssrc->connection);
          else
              res = FALSE;
          break ;
...
READ_TIMEOUT:
  {
    GST_ELEMENT_ERROR(mmssrc, RESOURCE, READ, ("ERROR_CONNECTION"), ("Connections timeouts with servers"));
    gst_buffer_unref (*buf);
    *buf = NULL;
  
    if (mmssrc->connection != NULL) {
      mms_close (mmssrc->connection);
      mmssrc->connection = NULL;
    }
    if (mmssrc->connection_h != NULL) {
      mmsh_close (mmssrc->connection_h);
      mmssrc->connection_h = NULL;
    }
    return GST_FLOW_UNEXPECTED;
  }
...
gst_mms_start (GstBaseSrc * bsrc)
{
  ...
  guint maxLimit_arate;
  guint maxLimit_vrate;
  ...
  if (mms->max_audio_rate)
    maxLimit_arate = mms->max_audio_rate;
  else
    maxLimit_arate = G_MAXINT;

  if (mms->max_video_rate)
    maxLimit_vrate = mms->max_video_rate;
  else
    maxLimit_vrate = G_MAXINT;
/**************************************/
/****** SAGEM : FIX mmsh latency ******/
/**************************************/

	GST_DEBUG_OBJECT (mms,
      "Trying mms_connect (%s) with bandwidth constraint of %d bps, max audio rate:%d, max video rate: %d",
      mms->uri_name, bandwidth_avail, mms->max_audio_rate, mms->max_video_rate);
	mms->connection = mms_connect (NULL, NULL, mms->uri_name, 
                                             bandwidth_avail,
                                             maxLimit_arate,
                                             maxLimit_vrate);
  ...
  GST_DEBUG_OBJECT (mms,
      "Trying mmsh_connect (%s) with bandwidth constraint of %d bps, max audio rate:%d, max video rate: %d",
      mms->uri_name, bandwidth_avail, mms->max_audio_rate, mms->max_video_rate);
      
   mms->connection_h = mmsh_connect (NULL, NULL, mms->uri_name, 
                                             bandwidth_avail,
                                             maxLimit_arate,
                                             maxLimit_vrate);
...
//  /* SAGEM: pass max_audio_rate and max_video_rate arguments */
//  GST_DEBUG_OBJECT (mms,
//      "Trying mms_connect (%s) with bandwidth constraint of %d bps, max audio rate:%d, max video rate: %d",
//      mms->uri_name, bandwidth_avail, mms->max_audio_rate, mms->max_video_rate);
//  mms->connection = mms_connect (NULL, NULL, mms->uri_name, 
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case ARG_CONNECTION_SPEED:
      ...
      break;
    case ARG_MAX_AUDIO_RATE:
      mmssrc->max_audio_rate = g_value_get_uint (value) * 1000;
      break;
    case ARG_MAX_VIDEO_RATE:
      mmssrc->max_video_rate = g_value_get_uint (value) * 1000;
      break;
    default:
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case ARG_CONNECTION_SPEED:
      ...
      break;
    case ARG_MAX_AUDIO_RATE:
      g_value_set_uint (value, mmssrc->max_audio_rate / 1000);
      break;
    case ARG_MAX_VIDEO_RATE:
      g_value_set_uint (value, mmssrc->max_video_rate / 1000);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
