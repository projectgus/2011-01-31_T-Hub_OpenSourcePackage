 ...
 *
 * Modified by Sagemcom under GPL license on 29/01/2008
 * Copyright (c) 2010 Sagemcom All rights reserved:
 * Author: David Giguet
 * Fix bug audio, the DMA was not flushing the expected audio buffer.
 * Create a pause function to correctly stop and restart stream.
 */
...
//SAGEM
#if defined(CONFIG_ULOG_HOOKS) && defined(CONFIG_ULOG_DMA)
#include <ulog/ulog.h>
#define DO_ULOG
#endif
//end SAGEM
...
#ifdef DO_ULOG
	ulog(ULOG_KERNEL_DMA_TASKLET_START,(unsigned int)arg);
	ulog(ULOG_KERNEL_DMA_TASKLET_CURR_BUF,(unsigned int)chnl_info->curr_buf);
#endif

	while (request_t.bd_done == 0) {

#ifdef DO_ULOG
	ulog(ULOG_KERNEL_DMA_TASKLET_BD_NUM,(unsigned int)data_priv->buf_tail);
#endif
...
#ifdef DO_ULOG
			ulog(ULOG_KERNEL_DMA_TASKLET_ERROR,0);
#endif
		}
