 ...
 * Add Init of usb and audio clock.
...
int __init mxc_clocks_init(void)
{
	...
	/* Init CCM clocks */
	__raw_writel(0x49FCFE7F, MXC_CCM_PDR1);
	__raw_writel(0x040C2403, MXC_CCM_UPCTL);
	__raw_writel(0x000002C6, MXC_CCM_COSR);
	...
	/*
	 * With the SPLL back up, turn on the console uart clock again so
	 * synchronous printk (with printascii patch) will continue to
	 * work.
	 */
