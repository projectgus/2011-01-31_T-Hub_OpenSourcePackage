 ...
 *
 * Modified by Sagemcom under GPL license on 24/09/2008
...
snd_pcm_sframes_t snd_pcm_oss_write3(struct snd_pcm_substream *substream, const char *ptr, snd_pcm_uframes_t frames, int in_kernel)
{
	...
	if (runtime->status->state == SNDRV_PCM_STATE_XRUN)
	{
		ulog(ULOG_OSS_WRITE_RECOVER_FROM_XRUN,__LINE__) ;
	}
#endif
...
#if defined(DO_ULOG)
	ulog(ULOG_OSS_WRITE3_DONE,__LINE__) ;
#endif
	return ret;
}
...
snd_pcm_sframes_t snd_pcm_oss_read3(struct snd_pcm_substream *substream, char *ptr, snd_pcm_uframes_t frames, int in_kernel)
{
	...
	if (runtime->status->state == SNDRV_PCM_STATE_XRUN)
	{
		ulog(ULOG_OSS_READ_RECOVER_FROM_XRUN,__LINE__) ;
	}
#endif
...
#if defined(DO_ULOG)
	ulog(ULOG_OSS_READ3_DONE,__LINE__) ;
#endif
...
static int snd_pcm_oss_get_formats(struct snd_pcm_oss_file *pcm_oss_file)
{
	...
	if (atomic_read(&substream->mmap_count))
		...
		direct = substream->oss.setup.direct;
	if (!direct)
		return AFMT_MU_LAW | AFMT_A_LAW | AFMT_U8 |
...
#if defined(DO_ULOG)
	ulog(ULOG_OSS_GET_SPACE,(unsigned int)info.bytes) ;
#endif
...
#if defined(DO_ULOG)
	ulog(ULOG_OSS_GET_SPACE_DONE,(unsigned int)info.bytes) ;
#endif
...
#ifdef OSS_DEBUG
	printk("%s - Open device 0x%x",__FUNCTION__,iminor(inode));
#endif
...
#ifdef OSS_DEBUG
	printk("%s - Release 0x%x",__FUNCTION__,iminor(inode));
#endif
...
static long snd_pcm_oss_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (((cmd >> 8) & 0xff) != 'P')
		return -EINVAL;
#ifdef OSS_DEBUG
	printk("%s - cmd=0x%x arg=0x%x",__FUNCTION__,cmd,(unsigned int)arg);
#endif
	switch (cmd) {
