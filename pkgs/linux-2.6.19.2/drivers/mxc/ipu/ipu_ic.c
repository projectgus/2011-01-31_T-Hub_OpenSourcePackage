 ...
 *
 * Modified by Sagemcom under GPL license on 10/11/2007 
 * Copyright (c) 2010 Sagemcom All rights reserved:
 * - Added suspend resume functions of TFT screen according to homescreen V4 specification.
 * - Power Management - Modified mxc ipu (display) suspend and resume code.
...
void _ipu_ic_init_pp(ipu_channel_params_t * params)
{
	...
	/* Setup horizontal resizing */
    /* To enable upscaling to outputs wider than 720 pixels, we process 
    half images. Therefore the upscale ratios also have to be 
    calculated based on the half image size values or we will get
    slightly wrong resize coefficients */

	_calc_resize_coeffs(params->mem_pp_mem.in_width/2,
			    params->mem_pp_mem.out_width/2,
			    &resizeCoeff, &downsizeCoeff);
	reg |= (downsizeCoeff << 14) | resizeCoeff;
