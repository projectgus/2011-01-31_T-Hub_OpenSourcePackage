 ...
 *
 * Modified by Sagemcom under LGPL license on 22/04/2009 
    ...
    const char *mimetype, const char *fieldname, ...)
{
  ...
  if (context != NULL && context->width != -1) {
	...
	"width", G_TYPE_INT, context->width ? context->width : 16,
        "height", G_TYPE_INT, context->height ? context->height : 16,
    ...
    AVCodecContext * context, gboolean encode)
{
  ...
  switch (codec_id) {
	...
	/* SAGEM: Add vorbis support in GstFFMpeg */
...
#if PIX_FMT_YUV422 != PIX_FMT_YUYV422
          case PIX_FMT_YUV422:
#endif
    ...
    case CODEC_ID_AAC:
#if CODEC_ID_AAC != CODEC_ID_MPEG4AAC
    ...
    case CODEC_ID_BMP:
      caps = gst_ff_vid_caps_new (context, codec_id, "image/bmp", NULL);
      break;
...
#if defined(CODEC_ID_ADPCM_IMA_ISS)
    case CODEC_ID_ADPCM_IMA_ISS:
#endif /* CODEC_ID_ADPCM_IMA_ISS */
...
#if defined(CODEC_ID_ADPCM_EA_MAXIS_XA)
    case CODEC_ID_ADPCM_EA_MAXIS_XA:
#endif /* CODEC_ID_ADPCM_EA_MAXIS_XA */
...
#if defined(SAMPLE_FMT_DBL)
    case SAMPLE_FMT_DBL:
      integer = FALSE;
      bpp = 64;
      break;
#endif /* SAMPLE_FMT_DBL */
    default:
      /* .. */
      break;
  }
...
gst_ffmpeg_formatid_to_caps (const gchar * format_name)
{
  ...
  if (!strcmp (format_name, "mpeg")) {
    caps = gst_caps_from_string ("video/mpeg,systemstream=true; video/mpeg2");
    ...
    caps = gst_caps_from_string ("video/x-ms-asf; video/x-ms-neuf; audio/x-ms-asf");
  } else if (!strcmp (format_name, "avi")) {
    caps = gst_caps_from_string ("video/x-msvideo; video/x-ms-avi; video/avi");
    ...
    caps = gst_caps_from_string ("application/x-id3; audio/mpeg");
    ...
    caps =
        ...
        ("application/x-3gp; video/quicktime; audio/x-m4a; video/mp4");
  } else if (!strcmp (format_name, "mov")) {
    caps = gst_caps_new_simple ("video/quicktime", NULL);
  } else if (!strcmp (format_name, "mp4")) {
    caps = gst_caps_from_string ("video/quicktime; video/mp4");
    ...
    AVOutputFormat * plugin)
{
  ...
  if (!strcmp (format_name, "mp4")) {
    ...
    *audio_codec_list = mp4_audio_list;
    ...
    static enum CodecID mpeg_audio_list[] = { 
#if defined(CODEC_ID_MP1)
      CODEC_ID_MP1,
#endif /* CODEC_ID_MP1 */
      CODEC_ID_MP2,
...
gst_ffmpeg_caps_to_codecid (const GstCaps * caps, AVCodecContext * context)
{
  ...
  if (!strcmp (mimetype, "video/x-raw-rgb") ||
    ...
    id = CODEC_ID_MJPEGB;
    video = TRUE;
  } else if (!strcmp (mimetype, "image/gif")) {
    id = CODEC_ID_GIF;
    video = TRUE;
  } else if (!strcmp (mimetype, "image/bmp")) {
    id = CODEC_ID_BMP;
    video = TRUE;
  } else if (!strcmp (mimetype, "image/tiff")) {
    id = CODEC_ID_TIFF;
    video = TRUE;
