  ...
  switch (h->flags) {
    case URL_RDONLY:
    {
      ...
      switch (whence) {
        ...
        case AVSEEK_SIZE:
        {
	    /* SAGEM: Implement AVSEEK_SIZE to enable streaming in pull mode */
	    GstFormat format = GST_FORMAT_BYTES;
	    gint64 duration;
		...
		if (gst_pad_query_duration (GST_PAD_PEER (info->pad), &format,
					    &duration))
		{
		    GST_DEBUG("AVSEEK_SIZE got %lld", duration);

		    /* Unknown size, streaming enabled */
		    if (duration < 0)
			newpos = 0;
		    else
			newpos = duration;
        ...
        default:
	  GST_DEBUG("Unkown seek mode %i", whence);
          break;
      }
