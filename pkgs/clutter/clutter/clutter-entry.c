 ...
 *
 * Modified by Sagemcom under LGPL license on 09/09/2008 
...
static gint
bytes_to_offset (const gchar *text, gint index)
{
  gchar *c = NULL;
  gint i, j, len;
...
void
clutter_entry_set_cursor_positionx(ClutterEntry *entry,gint x)
{
ClutterEntryPrivate  *priv;
gint                  index_;
...
gboolean trailing;
gboolean  is_inlayout;
is_inlayout=pango_layout_xy_to_index(priv->layout,x*PANGO_SCALE,priv->cursor_pos.y*PANGO_SCALE,&index_,&trailing);
//printf("is_inlayout %d index_ %d\n",is_inlayout,index_);
if (!priv->text_visible && priv->priv_char)
 ...
 if (priv->text_visible)
{
        priv->position = bytes_to_offset (priv->text, index_);
	//printf("ouai  \n");
}
      else
        priv->position = index_ / priv_char_bytes;
