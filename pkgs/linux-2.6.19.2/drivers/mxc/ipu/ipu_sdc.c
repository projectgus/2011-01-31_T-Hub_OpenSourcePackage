 ...
 *
 * Modified by Sagemcom under GPL license on 15/10/2007 
 * Copyright (c) 2010 Sagemcom All rights reserved:
 * -Deliver traces ulog for interrupts, task switches and mxc_v4l2_output
 * - Backlight pwm frequency is set to 100 kHz. This avoids flicker of the display.
 * - Add support for dataimage screen for HSV4 only.
 * - Screen is now completly black when in idle mode.
 * - Added robusness in screen init when the LCD is already initialise
 * - mxcfb & ipu_sdc : backlight control debugged, now contrast and blanking works using a ioctl for control a a ioctl for hardware drive.
 */
			   ...
			   ipu_di_signal_cfg_t sig)
{
	...
	// patch change force SDC enable
	ipu_conf = __raw_readl(IPU_CONF);
	if (ipu_conf & IPU_CONF_SDC_EN)
	{
			      ...
			      uint32_t color_key)
{
    ...
    #define IPU_SDC_MAX_BRIGHTNESS_VALUE 255
    #define IPU_SDC_MIN_BRIGHTNESS_VALUE 0
#else
    #define IPU_SDC_MAX_BRIGHTNESS_VALUE 255
    #define IPU_SDC_MIN_BRIGHTNESS_VALUE 0
#endif
...
int32_t ipu_sdc_set_brightness(uint8_t value)
{
   uint8_t _value = 0;
#if defined(DO_ULOG)
    ulog(ULOG_V4L2OUT_IPU_SDC_SET_BRIGHTNESS, 0) ;
#endif
   _value =  (value > IPU_SDC_MAX_BRIGHTNESS_VALUE) ? IPU_SDC_MAX_BRIGHTNESS_VALUE : value;
   _value =  (value < IPU_SDC_MIN_BRIGHTNESS_VALUE) ? IPU_SDC_MIN_BRIGHTNESS_VALUE : value;

#if defined(CONFIG_MACH_MX31LSV0)
   //CLD: BUG: switch off screen, no mean to restart it... check screen clocking.
#elif defined(CONFIG_MACH_MX31HSV1)  
   if ( get_board_type() >= BOARD_TYPE_V4_HOMESCREEN_GENERIC ) {
      /* DataImage */
      __raw_writel(0x03000000UL | (255 - _value) << 16, SDC_PWM_CTRL);
   }
   else
      __raw_writel(0x03000000UL | (255 - _value) << 16, SDC_PWM_CTRL); /* CLD: 24,25 = 11: 3 = cc_en + Pixclock (100 kHz) */
#else
   __raw_writel(0x03000000UL |  _value << 16, SDC_PWM_CTRL);
#endif
   return 0;
}
...
{
#if defined(DO_ULOG)
    ulog(ULOG_V4L2OUT_IPU_SDC_BG_INIT, 0) ;
#endif
	uint32_t reg;
	(void)params;
