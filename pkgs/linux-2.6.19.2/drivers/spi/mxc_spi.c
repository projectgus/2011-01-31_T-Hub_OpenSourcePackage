 ...
 *
 * Modified by Sagemcom under GPL license on 08/06/2009 
 * Copyright (c) 2010 Sagemcom All rights reserved:
 *       - add of spi protocol driver in drivers/spi/spi_dect*
 *       - SPI Controller driver bug fix. Reset was not done properly at boot.
 *       - Corrected wrong register address value for TESTREG in mxc_spi.h. Was 0xC0 instead of 0x1C0.
 */
...
#include <asm/arch/mxc_spi_mx27.h>
#else
...
static void spi_put_tx_data(void *base, unsigned int val)
{
	...
	//printk("spi_put_tx_data: writing value 0x%x in host data register, writing 0x%x in host ctrl register.\n", val,ctrl_reg);
...
void mxc_spi_chipselect(struct spi_device *spi, int is_active)
{
	...
	struct mxc_spi_xfer *ptransfer=NULL;
	struct mxc_spi_unique_def *spi_ver_def=NULL;
	unsigned int ctrl_reg=0;
	...
	dev_dbg(&spi->dev, "mxc_spi_chipselect: bit_per_word=%d, initial ctrl_reg value = 0x%x\n",xfer_len,ctrl_reg);
	...
	dev_dbg(&spi->dev, "mxc_spi_chipselect: written ctrl_reg value = 0x%x\n",ctrl_reg);
	...
	if(spi->master->bus_num == 1){
		...
		if(cd->spi_loopback){
			__raw_writel(MXC_CSPITEST_LBC, master_drv_data->base + MXC_CSPITEST);
		}else{
			__raw_writel(0, master_drv_data->base + MXC_CSPITEST);
		}
...
static irqreturn_t mxc_spi_isr(int irq, void *dev_id)
{
	...
	if (status & MXC_CSPISTAT_RR) {
		...
		u32 rx_tmp;
		if(master_drv_data->mxc_bitbang.master->bus_num == 1){
			rx_tmp = __le16_to_cpu(spi_get_rx_data(master_drv_data->base));
		}else{
			rx_tmp = spi_get_rx_data(master_drv_data->base);
		}
	...
	//printk("mxc_spi_isr: Got IRQ for SPI, status_reg=0x%x, value=0x%hx\n",status, *( (short *) master_drv_data->transfer.rx_buf));
	(master_drv_data->transfer.count)--;
	/* Handle the Tx now */
	
	if (master_drv_data->transfer.count) {
		if (master_drv_data->transfer.tx_buf) {
			u32 tx_tmp = master_drv_data->transfer.tx_get(master_drv_data);
			if(master_drv_data->mxc_bitbang.master->bus_num == 1){
				spi_put_tx_data(master_drv_data->base, __cpu_to_le16(tx_tmp));
			}else{
...
int mxc_spi_setup(struct spi_device *spi)
{
	...
	dev_dbg(&spi->dev,"%s: mode %d, %u bit per word, %d hz\n", __FUNCTION__,
...
/*!
 * This function initialize the current SPI device for transfer.
 *
 * @param        spi	the current SPI device.
 * @param        t	Message to transfer.
 *
 */
int mxc_spi_setup_transfer(struct spi_device *spi, struct spi_transfer *t){
	printk("%s() %d\n", __func__, __LINE__);
	...
	if(!t){
		/* TODO
		 * restaure spi board params if t is NULL. called by bitbang_work.
		 * */
		return 0;
	}

	if(spi->max_speed_hz < 0){
		dev_err(&spi->dev, "Bad max_speed_hz\n");
		return -EINVAL;
	}
	if(spi->bits_per_word != 8 &&
			spi->bits_per_word != 16 &&
			spi->bits_per_word != 32){
		dev_err(&spi->dev, "Bad bit per word\n");
		return -EINVAL;
	}

	spi->max_speed_hz = t->speed_hz;
	spi->bits_per_word = t->bits_per_word;
	/*t->cs_change = 1; force bitbang to call chipselect */
	return 0;
}
...
int mxc_spi_transfer(struct spi_device *spi, struct spi_transfer *t)
{
	...
	if ((master_drv_data->transfer.tx_buf == NULL) && (master_drv_data->transfer.rx_buf == NULL))
	{
		return -1;
	}
	...
	if (t->tx_buf == NULL)
	{
		master_drv_data->transfer.tx_buf = &dummy_buffer;
	}
	
	if (t->rx_buf == NULL)
	{
		master_drv_data->transfer.rx_buf = &dummy_buffer;
	}
	
	dev_dbg(&spi->dev, "mxc_spi_transfer[%d] tx buffer 0x%p, rx buffer 0x%p, bufs len %d\n",
		transfer_count,t->tx_buf, t->rx_buf, t->len);
	...
	dev_dbg(&spi->dev, "mxc_spi_transfer[%d] Data sent, tx buffer 0x%p, rx buffer 0x%p, transfer_count %d, value = 0x%hx\n",
		transfer_count, master_drv_data->transfer.tx_buf, master_drv_data->transfer.rx_buf, master_drv_data->transfer.count,
		*( (short*) master_drv_data->transfer.rx_buf));
	if (t->rx_buf != NULL)
	{
		dev_dbg(&spi->dev, "mxc_spi_transfer[%d] rx buffer value = 0x%hx\n",transfer_count,*( (short * ) t->rx_buf) );
	}
...
static int mxc_spi_probe(struct platform_device *pdev)
{
	...
	__raw_writel(0, master_drv_data->base + MXC_CSPICTRL); /* SAGEM Added by LEROYO. Performs a reset of the CSPI module */
	udelay(100); /* Arbitrary value for Reset timing */
	...
	dev_info(&pdev->dev, "CLOCK %s Rate %luHz SPI_REV 0.%d\n",
		"cspi_clk", master_drv_data->spi_ipg_clk, spi_ver);
...
{
	pr_info("Registering the MXC SPI Controller Driver\n");
...
{
	pr_info("Unregistering the MXC SPI Controller Driver\n");
