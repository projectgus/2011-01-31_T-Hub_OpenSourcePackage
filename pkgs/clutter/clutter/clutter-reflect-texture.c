 ...
 *
 * Modified by Sagemcom under LGPL license on 05/07/2010
...
//version clutter 7.0
    if(!clutter_texture_is_tiled(CLUTTER_TEXTURE(parent_texture)))
    {
       ...
       */
      if (clutter_feature_available (CLUTTER_FEATURE_TEXTURE_NPOT)) /*CLUTTER_FEATURE_TEXTURE_RECTANGLE*/
	{
...
/*version openGL*/
	...
	GLfloat vertices[] = {qx1,qy2,qx1,qy1,qx2,qy2,qx2,qy1};     // 8 of vertex coords
	GLfloat TexCoords[] = {0, ty2,0, ty,tx,  ty2,tx,  ty};
	GLubyte Colors[] = {255,255,255,0 ,255,255,255,clutter_actor_get_opacity (CLUTTER_ACTOR(ctexture)) ,255,255,255,0 ,255,255,255,clutter_actor_get_opacity (CLUTTER_ACTOR(ctexture))};
...
GLubyte div=clutter_actor_get_opacity (CLUTTER_ACTOR(ctexture))/n_y_tiles;
GLubyte alpha;
GLubyte alpha_old=clutter_actor_get_opacity (CLUTTER_ACTOR(ctexture));


  for (x = 0; x < n_x_tiles; x++)
    {
      lasty = 0;
      alpha_old=clutter_actor_get_opacity (CLUTTER_ACTOR(ctexture));	
      alpha=clutter_actor_get_opacity (CLUTTER_ACTOR(ctexture))-div;
      for (y = 0; y < n_y_tiles; y++)
	{
	  gint actual_w, actual_h;
	  gint xpos, ypos, xsize, ysize, ywaste, xwaste;
	  
	  clutter_texture_bind_tile (CLUTTER_TEXTURE(parent_texture), (n_y_tiles-1-y)*n_x_tiles+x);
	  ...
	  clutter_texture_get_y_tile_detail (CLUTTER_TEXTURE(parent_texture), 
					   n_y_tiles-1-y, &ypos, &ysize, &ywaste);
	  ...
	  actual_h=actual_h*rheight/qheight;

	  qx1 = x1 + lastx;
	  qx2 = qx1 + ((qwidth * actual_w ) / pwidth );
	  
	  qy1 = y1 + lasty;
	  qy2 = qy1 + ((rheight * actual_h) / pheight );
	...
	GLfloat vertices[] = {qx1,qy2,qx1,qy1,qx2,qy2,qx2,qy1};     // 8 of vertex coords
	GLubyte indices[] = {4,1,3,3,1,2};
	GLfloat TexCoords[] = {0, 0,0, ty,tx,  0,tx, ty};
	//printf(" %d %d \n",alpha,alpha_old);
	GLubyte Colors[] = {255,255,255,alpha ,255,255,255,alpha_old ,255,255,255,alpha ,255,255,255,alpha_old};
	// activate and specify pointer to vertex array
	glEnableClientState(GL_VERTEX_ARRAY);
	glVertexPointer(2, GL_FLOAT, 0, vertices);
	glEnableClientState(GL_TEXTURE_COORD_ARRAY);
...
clutter_reflect_texture_paint (ClutterActor *self)
{
  ...
  */
   ...
   */  
  if (clutter_feature_available (CLUTTER_FEATURE_TEXTURE_NPOT) &&
      clutter_texture_is_tiled (CLUTTER_TEXTURE (parent_texture)) == FALSE)
    {
      target_type = CGL_TEXTURE_RECTANGLE_ARB;
      cogl_enable (COGL_ENABLE_TEXTURE_RECT | COGL_ENABLE_BLEND);//CGL->COGL
    }
  else
    {
  ...
  //modif clutter 7.0
  clutter_actor_get_allocation_coords(self, &x1, &y1, &x2, &y2);
...
clutter_reflect_texture_class_init (ClutterReflectTextureClass *klass)
{
  ...
  g_object_class_install_property (gobject_class,
                                   ...
                                   g_param_spec_int ("reflection-height",
                                                     ...
                                                     0, G_MAXINT,
                                                     0,
                                                     ((GParamFlags)(G_PARAM_CONSTRUCT | CLUTTER_PARAM_READWRITE))));
...
ClutterActor * clutter_reflect_texture_new (ClutterTexture *texture, gint reflection_height)
{
  ...
  ClutterActor* actor=(ClutterActor*)g_object_new (CLUTTER_TYPE_REFLECT_TEXTURE,"parent-texture", texture,"reflection-height", reflection_height,NULL);
  return actor;
}
