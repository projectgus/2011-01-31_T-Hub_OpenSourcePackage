...
void mxcuart_console_debug(const char *s, ...)
{
	struct uart_port *port = &mxc_ports[console_index]->port;
	volatile unsigned int status, oldcr1, oldcr2, oldcr3, cr2, cr3;
	char buf[128];
	va_list ap;
	int i, count;
	 ...
	 */
	for (i = 0; i < count; i++) {
		mxcuart_console_write_char(port, buf[i]);
		if (buf[i] == '\n') {
			mxcuart_console_write_char(port, '\r');
		}
	}
...
EXPORT_SYMBOL(mxcuart_console_debug);
/*!
