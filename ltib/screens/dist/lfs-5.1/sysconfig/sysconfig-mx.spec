...
if [ "$SYSCFG_START_PCMCIA" = "y" ]
then
	pcmcia=pcmcia
fi
if [ "$SYSCFG_START_BLUETOOTH" = "y" ]
then
	bluetooth=bluetooth
fi
...
all_services_r="bluetooth qtopia dhcpd smb boa dropbear portmap inetd settime network pcmcia modules depmod syslog local filesystems mount-proc-sys udev hostname"
