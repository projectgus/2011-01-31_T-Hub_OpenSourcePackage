		...
		if (handle == NULL) {
			if (enable) {
				if (audio_mixer_control.codec_capture_active) {
					pr_debug("%s - audio_mixer_control.codec_capture_active\n",__FUNCTION__);
					handle =
					    audio_mixer_control.
					    voice_codec_handle;
...
#ifdef CONFIG_MACH_MX27MEDIAPHONE
	leftdb = (left * 0xc) / OUTPUT_VOLUME_MAX;           // SAGEM max volume of 0xc measured by G. Bourgoin
	rightdb = (right * 0xc) / OUTPUT_VOLUME_MAX;	     // SAGEM max volume of 0xc measured by G. Bourgoin
#else
...
#ifdef CONFIG_MACH_MX27MEDIAPHONE
		audio_stream->ssi = SSI2; // ssi2 on imx27
#else
		audio_stream->ssi = SSI1;
#endif // CONFIG_MACH_MX27MEDIAPHONE
		audio_stream->dam_port = DAM_PORT_4;
...
#ifdef CONFIG_MACH_MX27MEDIAPHONE
		audio_control->mixer_mono_adder = MONO_ADD_LEFT_RIGHT;
#else
		audio_control->mixer_mono_adder = MONO_ADDER_OFF;
#endif // CONFIG_MACH_MX27MEDIAPHONE
	/* Default values for input and output */
#ifdef CONFIG_MACH_MX27MEDIAPHONE
	audio_control->input_volume = ((70 << 8) & 0xff00) | (70 & 0x00ff);       // default input volume has been validated by G. Bourgoin (70)
	audio_control->master_volume_out = ((67 << 8) & 0xff00) | (67 & 0x00ff);  // default output volume has been validated by G. Bourgoin (67)
#else
...
#ifdef CONFIG_MACH_MX27MEDIAPHONE
	/* 
	 * Create a new PCM instance with 1 playback substream on stereodac
	 */
