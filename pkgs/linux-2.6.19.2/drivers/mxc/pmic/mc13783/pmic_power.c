...
static int pmic_power_probe(struct platform_device *pdev)
{
   ...
   t_regulator_voltage z_voltage;
#endif
	printk(KERN_INFO "PMIC Power successfully probed\n");
#if defined(CONFIG_MACH_MX31HSV1)
   if (get_board_type() >= BOARD_TYPE_V3_HOMESCREEN_GENERIC ) {
      z_voltage.vmmc1 = VMMC1_1_8V ; // 1.8V for VMMC1
   } 
   else {
      z_voltage.vmmc1 = VMMC1_3V ; // 3V for SDIO1 + SDCard2 
   }
   pmic_power_regulator_set_voltage(REGU_VMMC1, z_voltage);
   
   if (get_board_type() >= BOARD_TYPE_V3_HOMESCREEN_GENERIC ) {
      z_voltage.vmmc2 = VMMC2_1_8V;
      pmic_power_regulator_set_voltage(REGU_VMMC2, z_voltage);
