...
gst_base_src_start (GstBaseSrc * basesrc)
{
  ...
  /* If size is smaller than blocksize, we reduce blocksize to the default value */
  if (size > 0)
  {
      if (size < DEFAULT_BLOCKSIZE)
      {
	  while (size < basesrc->blocksize)
	      basesrc->blocksize /= 2;
	  if (basesrc->blocksize == 0)
	      basesrc->blocksize = 1;
      }
      else if (size < basesrc->blocksize)
      {
	  basesrc->blocksize = DEFAULT_BLOCKSIZE;
      }

      GST_DEBUG_OBJECT(basesrc, "new blocksize size is %i", basesrc->blocksize);
  }
