 ...
 *
 * Modified by Sagemcom under GPL license on 09/11/2007 
 * Copyright (c) 2010 Sagemcom All rights reserved.
 *
 * Deliver traces ulog for interrupts, task switches and mxc_v4l2_output
 * Increase DMA size to 16M
 * Remove MBX zone
 * Apply a patch from Xvid to fix issue on H.264 decoding
 */
...
{
#if defined(DO_ULOG)
    ulog(ULOG_V4L2OUT_MXC_PF_IRQ_HANDLER, 0) ;
#endif
...
#if defined(DO_ULOG)
    ulog(ULOG_V4L2OUT_MXC_PF_INIT, 0) ;
#endif
...
{
#if defined(DO_ULOG)
    ulog(ULOG_V4L2OUT_MXC_PF_UNINIT, 0) ;
#endif
...
{
#if defined(DO_ULOG)
    ulog(ULOG_V4L2OUT_MXC_PF_REQBUFS, 0) ;
#endif
	int err;
	uint32_t buf_size;
	int i;
...
static int mxc_pf_open(struct inode *inode, struct file *filp)
{
	int i;
#if defined(DO_ULOG)
    ulog(ULOG_V4L2OUT_MXC_PF_OPEN, 0) ;
#endif
	if (open_count) {
		return -EBUSY;
	}
...
{
#if defined(DO_ULOG)
    ulog(ULOG_V4L2OUT_MXC_PF_RELEASE, 0) ;
#endif
			...
			unsigned int cmd, unsigned long arg)
{
	int retval = 0;
#if defined(DO_ULOG)
    ulog(ULOG_V4L2OUT_MXC_PF_IOCTL, cmd) ;
#endif
	switch (cmd) {
	case PF_IOCTL_INIT:
		{
...
static int mxc_pf_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	int res = 0;
#if defined(DO_ULOG)
    ulog(ULOG_V4L2OUT_MXC_PF_MMAP, 0) ;
#endif
