...
struct tslib_variance {
	...
	unsigned int flags;
#define VAR_PENDOWN		0x00000001 /* doing an down event */
#define VAR_LASTVALID		0x00000002 /* last data is availlable */
#define VAR_NOISEVALID		0x00000004 /* last is noise, maybe not */
#define VAR_SUBMITNOISE		0x00000008 /* last was not a noisy event, run event again */
};
	...
	while (count < nr) {
		...
		if (cur.pressure == 0) {
			...
			if (var->flags & VAR_PENDOWN) {
				var->flags |= VAR_SUBMITNOISE;
				/* if previous samp was noisy, use last ok position */
