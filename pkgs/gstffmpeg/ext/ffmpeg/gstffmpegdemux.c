...
gst_ffmpegdemux_averror (gint av_errno)
{
  ...
  switch (av_errno) {
    ...
    case AVERROR_NUMEXPECTED:
      ...
      break;
#if AVERROR_INVALIDDATA!=AVERROR_UNKNOWN
...
gst_ffmpegdemux_perform_seek (GstFFMpegDemux * demux, GstEvent * event)
{
  ...
  /* not explicitely pausing the task can deadlock when trying to grab
   * the stream lock */ 
  ...
  if (!res) {
    /* search failed. Go back to old segment. */
    memcpy (&seeksegment, &demux->segment, sizeof (GstSegment));
    res = gst_ffmpegdemux_do_seek (demux, &seeksegment);
  }
...
gst_ffmpegdemux_get_stream (GstFFMpegDemux * demux, AVStream * avstream)
{
  ...
  /* SAGEM: It's better to use framerate from demuxer instead of codec */
  if (ctx->codec_type == CODEC_TYPE_VIDEO)
  {
      if (avstream->time_base.den != 0 && avstream->time_base.num != 0 &&
	  (double)avstream->time_base.den / (double)avstream->time_base.num < 30.0)
      {
	  GST_DEBUG("Apply framerate from video codec %i/%i", avstream->time_base.den, avstream->time_base.num);
      gst_caps_set_simple(caps, "framerate", GST_TYPE_FRACTION, 
			  avstream->time_base.den, avstream->time_base.num, NULL);
      }
      else if ((double)ctx->time_base.den / (double)ctx->time_base.num > 30.0)
      {
	  GST_DEBUG("Change framerate from %i/%i to 25/1", ctx->time_base.den, ctx->time_base.num);
...
gst_ffmpegdemux_loop (GstFFMpegDemux * demux)
{
  ...
  if (res < 0) {
    if (url_feof(M_FFMPEG_PTR(demux->context->pb)) == 0) /* XXX: should we use url_feof() here ?? */
    {
...
gst_ffmpegdemux_register (GstPlugin * plugin)
{
  ...
  while (in_plugin) {
    ...
    /* no emulators */
    if ((!strncmp (in_plugin->long_name, "raw ", 4) && strncmp (in_plugin->long_name, "raw ac3", 7)) ||
