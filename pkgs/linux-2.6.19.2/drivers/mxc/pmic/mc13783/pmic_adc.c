...
#define ADC_IT_TIMEOUT (msecs_to_jiffies(200))
...
#ifdef DO_ULOG
	ulog(ULOG_ATLAS_CLBK_TSI_DONE,0);
#endif
}
...
{
#ifdef DO_ULOG
	ulog(ULOG_ATLAS_CLBK_ADCDONE,0);
#endif
	if (data_ready_adc_1) {
		complete(&adcdone_it);
	}
#ifdef DO_ULOG
	ulog(ULOG_ATLAS_CLBK_ADCDONE_DONE,0);
#endif
}
...
#ifdef DO_ULOG
	ulog(ULOG_ATLAS_CLBK_ADCBISDONE,0);
#endif
	if (data_ready_adc_2) {
		complete(&adcbisdone_it);
	}
#ifdef DO_ULOG
	ulog(ULOG_ATLAS_CLBK_ADCBISDONE_DONE,0);
#endif
}
...
#ifdef DO_ULOG
	ulog(ULOG_ATLAS_ADC_COMP_HIGH,0);
#endif
...
#ifdef DO_ULOG
	ulog(ULOG_ATLAS_ADC_COMP_HIGH_DONE,0);
#endif
}
...
static int pmic_adc_filter(t_touch_screen * ts_curr)
{
...
#ifdef DO_ULOG
	ulog(ULOG_ATLAS_ADC_FILTER_RESISTANCE,ts_curr->contact_resistance);
	ulog(ULOG_ATLAS_ADC_FILTER_X1,ts_curr->x_position1);
	ulog(ULOG_ATLAS_ADC_FILTER_X2,ts_curr->x_position2);
	ulog(ULOG_ATLAS_ADC_FILTER_X3,ts_curr->x_position3);
	ulog(ULOG_ATLAS_ADC_FILTER_Y1,ts_curr->y_position1);
	ulog(ULOG_ATLAS_ADC_FILTER_Y2,ts_curr->y_position2);
	ulog(ULOG_ATLAS_ADC_FILTER_Y3,ts_curr->y_position3);
	ulog(ULOG_ATLAS_ADC_PREV_X0,prev_x[0]);
	ulog(ULOG_ATLAS_ADC_PREV_Y0,prev_y[0]);
	ulog(ULOG_ATLAS_ADC_PREV_X1,prev_x[1]);
	ulog(ULOG_ATLAS_ADC_PREV_Y1,prev_y[1]);
#endif
	...
	if (ts_curr->contact_resistance == 0) {
		...
		filt_count = 0;
#ifdef DO_ULOG
	ulog(ULOG_ATLAS_ADC_X,ts_curr->x_position);
	ulog(ULOG_ATLAS_ADC_Y,ts_curr->y_position);
#endif
		return 0;
	}
	...
	if (filt_count < FILTER_MIN_DELAY) {
		...
		filt_count++;
#ifdef DO_ULOG
		ulog(ULOG_ATLAS_ADC_FILTER_COUNT,filt_count);
#endif
	} else {
...
PMIC_STATUS mc13783_adc_convert(t_adc_param * adc_param)
{
	...
	unsigned int l_result = 0, temp = 0;
...
#ifdef DO_ULOG
	ulog(ULOG_ATLAS_ADC_CONVERT1,adc_param->wait_tsi);
#endif
...
#ifdef DO_ULOG
	ulog(ULOG_ATLAS_ADC_CONVERT2,adc_param->read_ts);
#endif
	/* CONFIGURE ADC REG 0 */
	adc_0_reg = 0;
	adc_1_reg = 0;
	...
	if (adc_param->read_ts == false) {
		...
		adc_1_reg = adc_1_reg | 0x084000;
#endif // CONFIG_MACH_MX27MEDIAPHONE || CONFIG_MACH_MX31HSV1
	}

#ifdef DO_ULOG
	ulog(ULOG_ATLAS_ADC_CONVERT3,use_bis);
#endif
	...
	if (use_bis == 0) {
		...
		l_result=wait_for_completion_interruptible_timeout(&adcdone_it,ADC_IT_TIMEOUT);
		pr_debug("adc done result = %d \n",l_result);
		data_ready_adc_1 = false;
	} else {
		data_ready_adc_2 = false;
		adc_1_reg |= ASC_ADC;
		data_ready_adc_2 = true;
		pr_debug("5 - Written Reg %i = %x\n", REG_ADC_1, adc_1_reg);
		...
		l_result=wait_for_completion_interruptible_timeout(&adcbisdone_it,ADC_IT_TIMEOUT);
		pr_debug("adc done bis result = %d \n",l_result);
		data_ready_adc_2 = false;
	}
	...
	if (adc_param->read_ts)
		CHECK_ERROR(pmic_write_reg(REG_ADC_0, 0x0,
				   ADC_TSMODE_MASK));
#ifdef DO_ULOG
	ulog(ULOG_ATLAS_ADC_CONVERT4,0);
#endif
...
PMIC_STATUS mc13783_adc_read_ts(t_touch_screen * ts_value, int wait_tsi)
{
	...
	pr_debug("TS_Y_MIN=%d\n",TS_Y_MIN);
	pr_debug("TS_Y_MAX=%d\n",TS_Y_MAX);
	pr_debug("x_position=%d \n",param.ts_value.x_position);
	pr_debug("y_position=%d \n",param.ts_value.y_position);
	pr_debug("x_position1=%d \n",param.ts_value.x_position1);
	pr_debug("y_position1=%d \n",param.ts_value.y_position1);
	pr_debug("x_position2=%d \n",param.ts_value.x_position2);
	pr_debug("y_position2=%d \n",param.ts_value.y_position2);
	pr_debug("x_position3=%d \n",param.ts_value.x_position3);
	pr_debug("y_position3=%d \n",param.ts_value.y_position3);
	pr_debug("contact_resistance=%d \n",param.ts_value.contact_resistance);
	/* check if x-y is ok */
