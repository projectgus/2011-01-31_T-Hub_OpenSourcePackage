...
//////////////////////////
  int    min_vrate    = -1;
//SAGEM  int    min_bw_left  = 0;
  int    stream_id;
//SAGEM  int    bandwitdh_left;
...
/* SAGEM: choose the best quality for the audio stream and video stream,
 * which don't overflow the choosen max limits*/
...
/////////////////////////////////////
  ...
  if ((video_stream == 0) && this->has_video) {
    for (i = 0; i < this->num_stream_ids; i++) {
      stream_id = this->stream_ids[i];
      switch (this->stream_types[stream_id]) {
        case ASF_STREAM_TYPE_VIDEO:
...
mmsh_t *mmsh_connect (mms_io_t *io, void *data, const char *url, int bandwidth, int max_audio_rate, int max_video_rate) {
  mmsh_t *this;
  GURI  *uri = NULL;
  GURI  *proxy_uri = NULL;
  char  *proxy_env;
...
static int get_media_packet (mms_io_t *io, mmsh_t *this) {
  ...
  if (ret = get_chunk_header(io, this)) {
    if(ret == -1)
    {
      lprintf ("mmsh: get_chunk_header failed: connection timeout\n");
      return -1;
    }
    switch (this->chunk_type) {
      case CHUNK_TYPE_END:
	...
	if (this->packet_length > CHUNK_SIZE) {
      ...
      lprintf ("mmsh: get_media_packet failed: connection timeout\n");
      return -1;
    }
    else {
...
/* return -1 if connection timeout with sever occurs , length read otherwise*/
int mmsh_read (mms_io_t *io, mmsh_t *this, char *data, int len) {
  ...
  while (total < len) {
	...
	if (packet_type == -1) {
	  lprintf ("mmsh: Connection timeout with server\n");
	  return -1;
