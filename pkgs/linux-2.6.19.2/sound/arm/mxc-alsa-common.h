 ...
 *
 * Modified by Sagemcom under GPL license on 04/07/2009 
...
typedef enum{
	STREAM_ID_STEREO_DAC,
	STREAM_ID_CAPTURE,
	STREAM_ID_VOICE_CODEC,
}STREAM_IDS;
...
extern int set_mixer_output_volume(PMIC_AUDIO_HANDLE handle, int volume, DEVICE_GAIN dev);
...
extern int get_mixer_output_volume(DEVICE_GAIN dev);
