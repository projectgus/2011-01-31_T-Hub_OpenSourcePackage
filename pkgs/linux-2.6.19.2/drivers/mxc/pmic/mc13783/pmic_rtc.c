...
static int pmic_rtc_remove(struct platform_device *pdev)
{
	...
	val = BITFVAL(MC13783_BATT_DAC_COIN_CH_EN,MC13783_BATT_DAC_COIN_CH_EN_DISABLED);
	mask = BITFMASK(MC13783_BATT_DAC_COIN_CH_EN);
	reg = REG_POWER_CONTROL_0;
	CHECK_ERROR(pmic_write_reg(reg, val, mask));
	printk(KERN_INFO "PMIC RTC coincell charger switched OFF\n");
...
{
	int ret = 0;
	struct class_device *temp_class=NULL;
	unsigned int val=0, mask=0, reg=0;
	...
	val = BITFVAL(MC13783_BATT_DAC_V_COIN, 0x100) | 
	        BITFVAL(MC13783_BATT_DAC_COIN_CH_EN,MC13783_BATT_DAC_COIN_CH_EN_ENABLED);
		mask = BITFMASK(MC13783_BATT_DAC_V_COIN) | BITFMASK(MC13783_BATT_DAC_COIN_CH_EN);
		reg = REG_POWER_CONTROL_0;
	CHECK_ERROR(pmic_write_reg(reg, val, mask));
	printk(KERN_INFO "PMIC RTC coincell charger switched ON\n");
