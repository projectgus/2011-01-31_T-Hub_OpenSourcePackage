...
#if 0 // Changed by LEROYO, W1 event is processed by eventMon
#if defined(CONFIG_EVTPROBE) || defined(CONFIG_EVTPROBE2)
	if ((SIOCGIWAP == cmd) && wrqu && is_zero_ether_addr(wrqu->ap_addr.sa_data)) {
		send_netlink_msg(SYSM_W1, 0, GFP_ATOMIC); //W1 OFF
	}	else {
		send_netlink_msg(SYSM_W1, 1, GFP_ATOMIC); //W1 ON
	}
#endif
#endif
