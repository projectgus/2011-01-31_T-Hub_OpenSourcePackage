 ...
 *
 * Modified by Sagemcom under GPL license on 25/07/2007 
...
// SAGEM: No hard-coded partitions
//	} else if (nr_parts < 0 && flash->parts) {
//		add_mtd_partitions(info->mtd, flash->parts, flash->nr_parts);
	} else
#endif
	{
#if defined(CONFIG_PARTITIONS_REQUIRED)
                printk(KERN_NOTICE "MXC flash: no partition ! "
		       "I won't register flash\n");
#else
