...
#ifdef U_LOG_ON
#include "ulog.h"
#include "ulog/ulog_symbols.h"
#else // U_LOG_ON
#define ulog(id, arg)		{}
#endif // U_LOG_ON

//#define SAGEM_DUMP_FILE
#ifdef SAGEM_DUMP_FILE
char *pg_buffer=NULL;
int pg_size=0;
#define PG_SIZE_MAX 44100*2*2*10
#endif
//end SAGEM
...
int snd_pcm_close(snd_pcm_t *pcm)
{
	int err;
	assert(pcm);
	ulog(ULOG_ALSA_PCM_CLOSE,(unsigned int)pcm);
...
#ifdef SAGEM_DUMP_FILE
	{
		...
		{
			FILE *fp=NULL;
			fp=fopen("/tmp/alsa.pcm","wb");
			if (fp != NULL)
			{
				printf("write file /tmp/alsa.pcm\n");
				fwrite(pg_buffer,1,pg_size,fp);
				fclose(fp);
				printf("/tmp/alsa.pcm has been created\n");
			}
			free(pg_buffer);
			pg_buffer=NULL;
		}
...
int snd_pcm_nonblock(snd_pcm_t *pcm, int nonblock)
{
	...
	ulog(ULOG_ALSA_PCM_NONBLOCK,nonblock);
...
{
	assert(pcm);
	ulog(ULOG_ALSA_PCM_RESUME,(unsigned int)pcm);
	if (CHECK_SANITY(! pcm->setup)) {
		SNDMSG("PCM not set up");
		return -EIO;
	}
...
int snd_pcm_prepare(snd_pcm_t *pcm)
{
	...
	ulog(ULOG_ALSA_PCM_PREPARE, (unsigned int)pcm);
...
int snd_pcm_drop(snd_pcm_t *pcm)
{
	...
	ulog(ULOG_ALSA_PCM_DROP, (unsigned int)pcm);
...
snd_pcm_sframes_t snd_pcm_writei(snd_pcm_t *pcm, const void *buffer, snd_pcm_uframes_t size)
{
	...
	ulog(ULOG_ALSA_PCM_WRITEI, (unsigned int)pcm);

#ifdef SAGEM_DUMP_FILE
	{
		int buffer_size=(pcm->msbits*pcm->channels*size)/8;
		if (pg_size==0)
		{
			printf("msbits=%d, channels=%d, size=%d buffer_size=%d\n",pcm->msbits,pcm->channels,size,buffer_size);
		}

		if ((pg_buffer!=NULL)&&(pg_size<PG_SIZE_MAX-buffer_size))
		{
			memcpy(&pg_buffer[pg_size],buffer,buffer_size);
			pg_size += buffer_size;
		}
	...
	if (ret == -EPIPE) ulog(ULOG_ALSA_PCM_WRITEI_UNDERRUN, (unsigned int)pcm);
	return ret;
}
...
snd_pcm_sframes_t snd_pcm_writen(snd_pcm_t *pcm, void **bufs, snd_pcm_uframes_t size)
{
	...
	ulog(ULOG_ALSA_PCM_WRITEN, (unsigned int)pcm);
	ret=_snd_pcm_writen(pcm, bufs, size);
	if (ret == -EPIPE) ulog(ULOG_ALSA_PCM_WRITEN_UNDERRUN, (unsigned int)pcm);
	return ret;
}
...
snd_pcm_sframes_t snd_pcm_readi(snd_pcm_t *pcm, void *buffer, snd_pcm_uframes_t size)
{
	...
	ulog(ULOG_ALSA_PCM_READI, (unsigned int)pcm);
	ret=_snd_pcm_readi(pcm, buffer, size);
	if (ret == -EPIPE) ulog(ULOG_ALSA_PCM_READI_OVERRUN, (unsigned int)pcm);
	return ret;
}
...
snd_pcm_sframes_t snd_pcm_readn(snd_pcm_t *pcm, void **bufs, snd_pcm_uframes_t size)
{
	...
	ulog(ULOG_ALSA_PCM_READN, (unsigned int)pcm);
	ret=_snd_pcm_readn(pcm, bufs, size);
	if (ret == -EPIPE) ulog(ULOG_ALSA_PCM_READN_OVERRUN, (unsigned int)pcm);
	return ret;
}
		 ...
		 snd_pcm_stream_t stream, int mode)
{
	...
	if (stream == SND_PCM_STREAM_PLAYBACK) {
		ulog(ULOG_ALSA_PCM_OPEN_PLAY, (unsigned int)pcmp);
	}
	else
		ulog(ULOG_ALSA_PCM_OPEN_RECORD, (unsigned int)pcmp);

#ifdef SAGEM_DUMP_FILE
	{
		
		pg_buffer=malloc(PG_SIZE_MAX);
		pg_size=0;
		if (pg_buffer==NULL)
		{
			printf("%s: Cannot allocate dump buffer\n",__FUNCTION__);
		}
...
{
	assert(pcm && params);
	ulog(ULOG_ALSA_PCM_SW_PARAMS_SET_START_THRESHOLD, val);
